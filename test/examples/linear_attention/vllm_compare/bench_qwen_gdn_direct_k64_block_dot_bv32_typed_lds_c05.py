#!/usr/bin/env python3
"""Fresh-process C0.5 benchmark: scalar transpose versus packed LDS layout."""
...
def _json_tail(output: str) -> object:
    for index in range(len(output.splitlines()) - 1, -1, -1):
...
def main() -> None:
    ...
    runners = {
        "persistent_scalar_lds_transpose": lambda t, s: _arm(
            t, args.warmup, args.repeat, s, "persistent_typed_block"
        ),
        "persistent_typed_lds_layout": lambda t, s: _arm(
            t, args.warmup, args.repeat, s, "persistent_typed_lds_layout"
        ),
        "native_triton_w0_control": lambda t, s: _native(t, args.warmup, args.repeat, s),
    }
    orders = [
        ("persistent_scalar_lds_transpose", "persistent_typed_lds_layout", "native_triton_w0_control"),
        ("persistent_typed_lds_layout", "native_triton_w0_control", "persistent_scalar_lds_transpose"),
        ("native_triton_w0_control", "persistent_scalar_lds_transpose", "persistent_typed_lds_layout"),
    ]
    ...
    if args.json:
        ...
        return
    for row in rows:
        print("typed_lds_c05," + ",".join(f"{key}={value}" for key, value in row.items()))
