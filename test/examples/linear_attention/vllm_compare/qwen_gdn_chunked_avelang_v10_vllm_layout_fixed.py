"""Qwen GDN v10 vLLM-layout structural chunk_gdr + v9 chunk_o.

This file is self-contained for the v10 experiment: it copies the tested v9
VxK kernels, then changes ``chunk_gdr`` to batch the K reduction for every
token in a chunk.  This follows the vLLM/Triton direction where a chunk is
processed as a small block operation instead of reducing one token at a time.
The pipeline remains:

    cumsum -> KKT -> solve -> w/u -> v10 chunk_gdr_chunk_vk -> v9 chunk_o_vk
...
V10_VLLM_LAYOUT_OPTIMIZATION_SUMMARY = {
    "target": "chunk_gdr + chunk_o",
    "mapping": "chunk_vk: block_id -> flattened (B,Hv,V block), thread_id.x -> V lane, thread_id.y -> K lane",
    "chunk_gdr_strategy": "batch all token offsets in one chunk into a shared [V, chunk, K-lane] reduction",
    ...
    "fallback": "v6 chunk_gdr fallback is preserved via use_parallel_chunk_gdr=False; v9-style chunk_o fallback is preserved via use_parallel_chunk_o=False",
}


@avelang.jit
def _qwen_gdn_chunk_gdr_bf16_kernel_v10_chunk_vk(
    ...
    partial_pred = al.make_shared((block_v, chunk_size, block_k), al.f32)
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # v10 structural change: compute all token offsets in this chunk first,
        # then reduce the K dimension for every offset in one shared-memory tree.
        # This mirrors vLLM/Triton chunk_delta_h's chunk-level block operation,
        # but keeps the current scalar Avelang lane mapping instead of using MFMA.
        for offset in al.range(chunk_size):
            ...
            partial_pred[lane_v, offset, lane_k] = partial
        ...
        for _step in al.range(k_reduce_steps):
            if lane_k < stride:
                for offset in al.range(chunk_size):
                    partial_pred[lane_v, offset, lane_k] = (
                        partial_pred[lane_v, offset, lane_k] + partial_pred[lane_v, offset, lane_k + stride]
                    )
            al.syncthreads()
            stride = stride >> 1
        ...
        if lane_k == 0:
            for offset in al.range(chunk_size):
                ...
                if value_valid and token_idx < num_tokens:
                    v_new = u[batch_idx, token_idx, value_head_idx, value_idx] - partial_pred[lane_v, offset, 0]
...
def _validate_vk_params(block_v: int, block_k: int) -> None:
    if block_v not in (1, 2, 4, 8, 16, 32, 64):
        raise ValueError(f"block_v must be 1, 2, 4, 8, 16, 32, or 64 for v10 chunk_vk chunk_gdr, got {block_v}.")
    if block_k not in (4, 8, 16, 32, 64, 128):
        raise ValueError(f"block_k must be 4, 8, 16, 32, 64, or 128 for v10 chunk_vk chunk_gdr, got {block_k}.")
...
def qwen_gdn_chunk_gdr_avelang_v10_vllm_layout(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    parallel_mode: str = "chunk_vk",
    block_v: int = 4,
    block_k: int | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Run chunk_gdr.  BF16 uses v10 chunk-batched VxK mapping by default."""
    ...
    if parallel_mode not in ("chunk_vk", "vk"):
        raise ValueError(f"parallel_mode must be 'chunk_vk' or 'vk' for v10 chunk_gdr, got {parallel_mode!r}.")
    ...
    _qwen_gdn_chunk_gdr_bf16_kernel_v10_chunk_vk[lambda: ((grid_size, 1, 1), (block_v, block_k, 1))](
        k,
        w,
        u,
        g,
...
def qwen_gdn_chunk_o_avelang_v9_vllm_layout(
    ...
    *,
    ...
    """Run chunk_o. BF16 uses v10 chunk-batched VxK mapping by default."""
...
def qwen_gdn_chunked_avelang_v10_vllm_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    h, vn, final_state = qwen_gdn_chunk_gdr_avelang_v10_vllm_layout(
        k,
        w,
        u,
        g_cumsum,
...
def qwen_gdn_chunked_avelang_v10_vllm_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _, output, _, _, final_state = qwen_gdn_chunked_avelang_v10_vllm_layout_full(
        q,
        k,
        v,
        g,
...
__all__ = [
    "V10_VLLM_LAYOUT_OPTIMIZATION_SUMMARY",
    "_qwen_gdn_chunk_gdr_bf16_kernel_v10_chunk_vk",
    "_qwen_gdn_chunk_o_bf16_kernel_v9_vk",
    "qwen_gdn_chunk_gdr_avelang_v10_vllm_layout",
    "qwen_gdn_chunk_o_avelang_v9_vllm_layout",
    "qwen_gdn_chunked_avelang_v10_vllm_layout_full",
