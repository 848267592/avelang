#!/usr/bin/env python3
"""R4-tail U/V-new LDS-mediated packet bridge candidate.

The only altered recurrence phase is pred-finalize -> U/V-new -> V-decay:
R4's two V1xT8 pred-partial planes are reduced in-place into a token-major
LDS bridge, then 128 static T1xV8 owners issue U/V-new b128 packets.  H,
W/K staging, FP32 state ownership, MFMA geometry, tail issue, and the
specialized V-decay/K update block-dot are inherited unchanged from R4-tail.
"""
...
BT = r4.BT
PLAN = "gfx942_bt64_bv32_joint_v4_tail_issue_u_vnew_lds_bridge"
_LDS_BRIDGE_PACKET_MODE = 2
...
def _make_launch(
    ...
    def launch() -> None:
        r4._qwen_gdn_persistent_recurrence_r4_joint_v4_kernel[
            ...
            True, _LDS_BRIDGE_PACKET_MODE, num_warps=2,
        )
...
def run_body(
    ...
    """Production body with all audit-only stores compiled out."""
    outputs = r0._allocate(int(k.shape[1]), initial_state, u)
...
def run_correctness_length(t: int, *, seed: int) -> dict[str, Any]:
    ...
    controls = {
        "lds_bridge_vs_p2_host_microscope": b0._run_p2_host_microscope(
            k, w, u, g, initial_state),
        "lds_bridge_vs_device_contract": b0._reference(k, w, u, g, initial_state),
    }
    torch.cuda.synchronize()
    comparisons = {
    ...
    return {
        ...
        "per_chunk": b0._per_chunk_rows(
            actual, controls["lds_bridge_vs_device_contract"]),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        / "rocprof_outputs/qwen_persistent_recurrence_r4_tail_issue_u_vnew_lds_bridge",
    )
    ...
    (args.out_dir / "lds_bridge_correctness.json").write_text(
    ...
    if not all(bool(result["pass"]) for result in results):
        raise SystemExit("R4-tail U/V-new LDS-bridge correctness gate failed")
