"""Stage 6U opt-in BT64 solve with FP32 compute and BF16 writeback.

The internal DAG, shared storage, recurrence and MFMA are copied from the
validated Stage 5B FP32 solve. Only the final global storage boundary changes:
for each `[chunk, value_head]`, it returns BF16 `X = (I + A)^-1` in the
original `[1,T,8,64]` layout. There is no fallback.
...
def _qwen_gdn_solve_hierarchical_bt64_fp32_compute_bf16_store_stage6u(
    ...
    for rep_zero in al.range(16):
        ...
        row = linear // BT
        col = linear - row * BT
        out[0, chunk_start + row, head_idx, col] = al.convert(0.0, al.bf16)
    al.syncthreads()
    ...
    for rep_diag_out in al.range(4):
        ...
        row = local // BLOCK
        col = local - row * BLOCK
        out[0, chunk_start + diag_block * BLOCK + row, head_idx, diag_block * BLOCK + col] = al.convert(
    ...
    if wave_id < 2:
        for r in al.range(4):
            out[0, chunk_start + (wave_id + 1) * BLOCK + lane_group * 4 + r, head_idx, wave_id * BLOCK + lane_col] = al.convert(
                x[wave_id + 4, lane_group * 4 + r, lane_col], al.bf16
            )
    if wave_id == 2:
        for r in al.range(4):
            out[0, chunk_start + 3 * BLOCK + lane_group * 4 + r, head_idx, 2 * BLOCK + lane_col] = al.convert(
    ...
    if wave_id == 0:
        for r in al.range(4):
            out[0, chunk_start + 2 * BLOCK + lane_group * 4 + r, head_idx, lane_col] = al.convert(
    ...
    if wave_id == 1:
        for r in al.range(4):
            out[0, chunk_start + 3 * BLOCK + lane_group * 4 + r, head_idx, BLOCK + lane_col] = al.convert(
                x[6, lane_group * 4 + r, lane_col], al.bf16
            )
    al.syncthreads()
    ...
    if wave_id == 0:
        for r in al.range(4):
            out[0, chunk_start + 3 * BLOCK + lane_group * 4 + r, head_idx, lane_col] = al.convert(
...
def qwen_gdn_solve_hierarchical_bt64_casted_bf16_solved_stage6u(a: torch.Tensor) -> torch.Tensor:
    """P-REF: current FP32 solve followed by a numeric BF16 cast."""
    from qwen_gdn_solve_bt64_hierarchical_fp32_v1 import qwen_gdn_solve_bt64_hierarchical_fp32_v1

    _validate_input(a)
    return qwen_gdn_solve_bt64_hierarchical_fp32_v1(a).to(torch.bfloat16)


def _qwen_gdn_solve_hierarchical_bt64_bf16_stage6u_launch_into(
    a: torch.Tensor, out: torch.Tensor
) -> torch.Tensor:
    """Diagnostic direct-out launcher used for prefill/reuse coverage."""
    num_tokens = _validate_input(a)
    if (
        out.dtype != torch.bfloat16
        or tuple(out.shape) != tuple(a.shape)
        or not out.is_cuda
        or not out.is_contiguous()
        or out.device != a.device
    ):
        raise ValueError("Stage 6U P0 output must be contiguous BF16 with the input shape/device.")
    num_chunks = num_tokens // BT
    _qwen_gdn_solve_hierarchical_bt64_fp32_compute_bf16_store_stage6u[
        lambda: ((num_chunks * HEADS, 1, 1), (WORKGROUP, 1, 1))
    ](
        a,
        out,
        num_tokens,
...
def qwen_gdn_solve_hierarchical_bt64_bf16_solved_stage6u(a: torch.Tensor) -> torch.Tensor:
    """P0: solve in FP32 and write the final fixed-layout result as BF16."""
    _validate_input(a)
    out = torch.empty(a.shape, dtype=torch.bfloat16, device=a.device)
    return _qwen_gdn_solve_hierarchical_bt64_bf16_stage6u_launch_into(a, out)
