"""Qwen GDN v29 MFMA32 pred accumulator serialization experiment.
...
* MFMA32 accumulator values are serialized in lane-major LDS before the
  cross-wave pred reduction. The epilogue reloads them through the inverse
  MFMA32 mapping instead of writing a permuted ``pred_partial`` tile.
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v29_mfma32_pred_epilogue_streaming(
    ...
    # The same 8 KiB footprint as pred_partial[2, 32, 32], but this layout
    # keeps each MFMA32 output fragment contiguous while it is serialized.
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        for token_tile in al.range(2):
            ...
            for rep_corr in al.range(8):
                ...
                # Invert the MFMA32 accumulator mapping used by the producer:
                # row = ((acc_i & 3) * 8) + (lane & 7)
                # col = ((acc_i >> 2) * 8) + ((lane >> 3) * 4) + (lane >> 4)
...
def qwen_gdn_fused_chunk_gdr_full_avelang_v29_mfma32_pred_epilogue_streaming(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v29_mfma32_pred_epilogue_streaming[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v29_mfma32_pred_epilogue_streaming")
    launch()
...
def _smoke_main() -> None:
    ...
    h, final_state = qwen_gdn_fused_chunk_gdr_full_avelang_v29_mfma32_pred_epilogue_streaming(
        k,
        w,
        u,
        gdr_decay,
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_chunk_gdr_full_avelang_v29_mfma32_pred_epilogue_streaming(k, w, u, gdr_decay, gdr_g_last_exp, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v29_pred_epilogue_streaming_ms={latency:.6f}")
