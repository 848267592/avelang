#!/usr/bin/env python3
"""R3 joint_v3 full recurrence using the shared R1 semantic kernel.
...
import repro_qwen_gdn_persistent_recurrence_r3_joint_v3 as joint_v3


BT = r1.BT
LOWERING = "gfx942_bt64_bv32_joint_v3"


def _set_r3_lowering() -> None:
...
def _make_launch(
    ...
    *,
    ...
    def launch() -> None:
        joint_v3._qwen_gdn_persistent_recurrence_r3_joint_v3_kernel[
...
def run_correctness_length(t: int, *, seed: int) -> dict[str, Any]:
    ...
    comparisons = {
        "r3_vs_r2": b0._compare(actual, r2_actual, label="r3_vs_r2"),
        "r3_vs_r1": b0._compare(actual, r1_actual, label="r3_vs_r1"),
        "r3_vs_p2": b0._compare(actual, microscope, label="r3_vs_p2_host_microscope"),
        "r3_vs_device_contract": b0._compare(actual, reference, label="r3_vs_device_contract"),
    }
...
def main() -> None:
    ...
    (args.out_dir / "r3_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
