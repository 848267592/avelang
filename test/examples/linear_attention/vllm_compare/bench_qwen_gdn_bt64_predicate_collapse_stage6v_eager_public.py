#!/usr/bin/env python3
"""ABBA eager-public benchmark for U1, Stage 6V V1, and native vLLM."""
...
def calls_for_t(t: int):
    q, k, v, g, beta, h0 = make_inputs(t, 2026073600 + t, "random", True)
    ...
    return {
        ...
        "v1": lambda: qwen_gdn_full_bt64_stage6v_predicate_collapse_eager(q, k, v, g, beta, **common),
...
def summarize(raw: list[dict[str, object]]):
    ...
    for t in sorted({int(row["T"]) for row in raw}):
        for reference in ("u1", "vllm"):
            values = [
                1000.0 * (indexed[(t, session, repeat, reference)] - indexed[(t, session, repeat, "v1")])
            ...
            ci_low, ci_high = bootstrap_ci(values, 2026073700 + t + len(reference))
            pairs.append({
                "T": t, "candidate": "v1", "reference": reference,
