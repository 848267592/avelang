...
def test_v16_2wave_chunk_gdr_matches_v14(t: int, with_initial_state: bool):
    chunk = 16
    _, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=16000 + t + int(with_initial_state))
    g_cumsum, w, u = _build_gdr_inputs(k, v, g, beta, chunk)

    h_ref, vn_ref, fs_ref = qwen_gdn_chunk_gdr_avelang_v14_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    h_new, vn_new, fs_new = qwen_gdn_chunk_gdr_avelang_v16_2wave_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
...
@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v16_full_forward_matches_v14(t: int, with_initial_state: bool):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=17000 + t + int(with_initial_state))
    scale = 128 ** -0.5
    ...
    out_new, fs_new = qwen_gdn_chunked_avelang_v16_mfma_layout(
        q,
        k,
        v,
        g,
