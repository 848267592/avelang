#!/usr/bin/env python3
"""R5 joint_v5: full typed W/K packet-to-dot recurrence pipeline.
...
@avelang.jit
def _qwen_gdn_persistent_recurrence_r5_joint_v5_kernel(
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # R5 preserves R2's interleaved next-chunk issue. The joint_v5 plan
...
def _maybe_dump_hsaco(launch: Callable[[], None]) -> None:
    ...
    target = _HSACO_DUMP_DIR / f"{_qwen_gdn_persistent_recurrence_r5_joint_v5_kernel.fn.__name__}.hsaco"
    if target.exists():
        launch()
        return
    ...
    def wrapped_compile(self, src, target_info, options=None):
        ...
        if not dumped and "persistent_recurrence_r5_joint_v5_kernel" in src.fn.fn.__name__:
    ...
    if not dumped:
        raise RuntimeError("R5 joint_v5 HSACO dump requested, but no matching kernel compiled")
...
def _run_kernel(
    ...
    p1._set_c0_lowering()
    os.environ["AVELANG_PERSISTENT_RECURRENCE_LOWERING"] = "gfx942_bt64_bv32_joint_v5"
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r5_joint_v5_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "rocprof_outputs/qwen_persistent_recurrence_r5_joint_v5",
    )
    ...
    (args.out_dir / "r5_joint_v5_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
    ...
    if not all(bool(result["pass"]) for result in results):
        raise SystemExit("R5 correctness gate failed; do not run resource or performance collection.")
