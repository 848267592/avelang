"""Qwen GDN v29 pred-only BT64/BV32 MFMA32 epilogue optimization experiment.
...
Important
---------
This file starts from the v29 ISA-tuning pred-only kernel and changes only the
VN epilogue. The goal is to test whether source-level indexing/store changes
can reduce the large full-VN epilogue cost observed after MFMA32 pred.
...
MODE_FLAT_EPILOGUE = "flat_epilogue"
MODE_GROUPED_V4_EPILOGUE = "grouped_v4_epilogue"
MODE_FUSED_NO_VN_STORE = "fused_no_vn_store"

EPILOGUE_BASELINE = 0
EPILOGUE_FLAT = 1
EPILOGUE_GROUPED_V4 = 2
EPILOGUE_FUSED_NO_STORE = 3
...
_VARIANTS = {
    ...
    MODE_FLAT_EPILOGUE: dict(
        do_acc_unpack=True,
        do_reduce=True,
        store_full_vn=True,
        constant_state=False,
        precomputed_mapping=False,
        epilogue_kind=EPILOGUE_FLAT,
    ),
    ...
    # Profiling-only upper bound. It keeps MFMA32, accumulator unpack, partial
    # reduction, and one U load side effect, but does not materialize full VN.
    MODE_FUSED_NO_VN_STORE: dict(
        do_acc_unpack=True,
        do_reduce=True,
        store_full_vn=False,
        constant_state=False,
        precomputed_mapping=False,
        epilogue_kind=EPILOGUE_FUSED_NO_STORE,
    ),
}


@avelang.jit
def _qwen_gdn_pred_only_bf16_kernel_v29_mfma32_epilogue_opt(
                ...
                if store_full_vn:
                    if epilogue_kind == EPILOGUE_GROUPED_V4:
                        ...
                        for rep_out in al.range(8):
                            ...
                            if epilogue_kind == EPILOGUE_FLAT:
                    ...
                    # The U load keeps the useful epilogue dependency live while
                    # avoiding 1024 global VN stores per token tile.
                    if tid == 0:
                        pred_o_small = pred_partial[0, 0, 0] + pred_partial[1, 0, 0]
                        side_offset = (chunk_start + token_base) * (8 * 128) + value_head_idx * 128 + value_base
                        vn_flat[side_offset] = u_flat[side_offset] - pred_o_small
            else:
                if do_acc_unpack:
...
def _variant_flags(variant: str) -> dict[str, bool]:
    if variant not in _VARIANTS:
        raise ValueError(f"unknown v29 epilogue_opt variant {variant!r}; expected one of {sorted(_VARIANTS)}")
    return _VARIANTS[variant]


def qwen_gdn_pred_only_avelang_v29_mfma32_epilogue_opt(
    w: torch.Tensor,
    u: torch.Tensor,
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_pred_only_bf16_kernel_v29_mfma32_epilogue_opt[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_pred_only_bf16_kernel_v29_mfma32_epilogue_opt")
    launch()
...
def qwen_gdn_pred_only_avelang_v29_mfma32(
    ...
    *,
    ...
    return qwen_gdn_pred_only_avelang_v29_mfma32_epilogue_opt(
        w,
        u,
        initial_state,
...
def _smoke_main() -> None:
    ...
    vn = qwen_gdn_pred_only_avelang_v29_mfma32_epilogue_opt(w, u, initial_state, variant=args.variant)
    torch.cuda.synchronize()
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_pred_only_avelang_v29_mfma32_epilogue_opt(w, u, initial_state, variant=args.variant),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} variant={args.variant} v29_epilogue_opt_ms={latency:.6f}")
