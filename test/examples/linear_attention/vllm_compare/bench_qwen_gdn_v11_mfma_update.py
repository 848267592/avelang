...
from qwen_gdn_chunked_avelang_v10_vllm_layout_fixed import qwen_gdn_chunked_avelang_v10_vllm_layout
...
def stage_breakdown_v11(q, k, v, g, beta, initial_state, scale, use_update_mfma: bool, warmup: int, repeat: int):
    chunk = 16
    ...
    w, u = wu()
    def gdr(): return qwen_gdn_chunk_gdr_avelang_v11_mfma_layout(k, w, u, g_cumsum, initial_state=initial_state, chunk_size=chunk, use_mfma_chunk_gdr=True, block_v=16, block_k=64, use_clean_kernel=True, use_update_mfma=use_update_mfma)
    h, vn, _ = gdr()
    def chunk_o(): return qwen_gdn_chunk_o_avelang_v9_vllm_layout(q, k, vn, h, g_cumsum, scale=scale, chunk_size=chunk, use_parallel_chunk_o=True, chunk_o_parallel_mode="vk", chunk_o_block_v=4, chunk_o_block_k=16)
    return {
...
def run_t(t: int, warmup: int, repeat: int):
    ...
    scale = 128 ** -0.5
    print(f"target_shape,B=1,T={t},Hk=4,Hv=8,K=128,V=128,dtype=BF16,layout=[B,T,H,D]")

    f_vllm = lambda: call_vllm(q, k, v, g, beta, initial_state, scale)
    f_v10 = lambda: qwen_gdn_chunked_avelang_v10_vllm_layout(q, k, v, g, beta, initial_state=initial_state, scale=scale, chunk_size=4, prefer_optimized=True)
    f_v11_scalar = lambda: qwen_gdn_chunked_avelang_v11_mfma_layout(q, k, v, g, beta, initial_state=initial_state, scale=scale, chunk_size=16, use_mfma_chunk_gdr=True, use_clean_kernel=True, use_update_mfma=False)
    f_v11_mfma = lambda: qwen_gdn_chunked_avelang_v11_mfma_layout(q, k, v, g, beta, initial_state=initial_state, scale=scale, chunk_size=16, use_mfma_chunk_gdr=True, use_clean_kernel=True, use_update_mfma=True)

    out_vllm, fs_vllm = f_vllm()
    out_v11s, fs_v11s = f_v11_scalar()
    out_v11m, fs_v11m = f_v11_mfma()
    sync()

    result = {
        "T": t,
        "vllm_ms": time_fn(f_vllm, warmup, repeat),
        "v10_ms": time_fn(f_v10, warmup, repeat),
        "v11_scalar_ms": time_fn(f_v11_scalar, warmup, repeat),
        "v11_update_mfma_ms": time_fn(f_v11_mfma, warmup, repeat),
        "update_vs_scalar_out_err": max_err(out_v11m, out_v11s),
        "update_vs_scalar_fs_err": max_err(fs_v11m, fs_v11s),
        "v11_update_vs_vllm_out_err": max_err(out_v11m, out_vllm),
        "v11_update_vs_vllm_fs_err": max_err(fs_v11m, fs_vllm),
        "stages_scalar": stage_breakdown_v11(q, k, v, g, beta, initial_state, scale, False, max(2, warmup // 4), max(5, repeat // 4)),
        "stages_update_mfma": stage_breakdown_v11(q, k, v, g, beta, initial_state, scale, True, max(2, warmup // 4), max(5, repeat // 4)),
    }
    print(result)
    return result
...
def main():
    ...
    parser.add_argument("--T", type=int, nargs="*", default=[64,128,256,512, 1024, 2048])
    ...
    for r in rows:
        print(
            f"T={r['T']} vLLM={r['vllm_ms']:.4f}ms v10={r['v10_ms']:.4f}ms "
            f"v11_scalar={r['v11_scalar_ms']:.4f}ms v11_update_mfma={r['v11_update_mfma_ms']:.4f}ms "
            f"speedup_update_vs_scalar={r['v11_scalar_ms']/r['v11_update_mfma_ms']:.4f} "
            f"speedup_update_vs_v10={r['v10_ms']/r['v11_update_mfma_ms']:.4f} "
            f"vllm_over_update={r['vllm_ms']/r['v11_update_mfma_ms']:.4f} "
            f"out_err_scalar={r['update_vs_scalar_out_err']:.6g} fs_err_scalar={r['update_vs_scalar_fs_err']:.6g}"
        )
