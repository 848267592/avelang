"""Qwen GDN v29 pred-only token-split stage-all MFMA32 experiment.
...
@avelang.jit
def _qwen_gdn_pred_only_bf16_kernel_v29_token_split_stage_all_mfma32(
    ...
    w_bf16 = al.make_shared((2, 2, 32, 64), al.bf16)
    ...
    w_vec = al.view(
        w_bf16,
        al.i32,
        al.make_layout((2, 2, 32, 8, 4), (2 * 32 * 8 * 4, 32 * 8 * 4, 8 * 4, 4, 1)),
    )
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # ----------------------------------------------------
        # Stage W for both token tiles and both K halves before
        # pred_acc is created. This is the key change versus the
        # previous token_split version: pred_acc no longer lives
        # across W staging and barriers.
        #
        # total = 2 token_tiles * 2 khalves * 32 tokens * 64 K
        #       = 8192 bf16 elements
        # 128 threads * 64 reps = 8192
        # ----------------------------------------------------
        for rep_w in al.range(64):
            linear_w = tid + rep_w * WORKGROUP

            tile_w = linear_w // (2 * 32 * 64)
            rem0_w = linear_w - tile_w * (2 * 32 * 64)

            khalf_w = rem0_w // (32 * 64)
            rem1_w = rem0_w - khalf_w * (32 * 64)

            token_off_w = rem1_w // 64
            col_w = rem1_w - token_off_w * 64

            token_idx_w = chunk_start + tile_w * 32 + token_off_w
            global_k_w = khalf_w * 64 + col_w

            w_bf16[tile_w, khalf_w, token_off_w, col_w] = al.convert(
        ...
        #
        # It now only lives through the pure MFMA section and the final store.
        ...
        for khalf in al.range(2):
            for kpack in al.range(4):
                a_words = w_vec[wave_id, khalf, a_row, kpack]
...
def qwen_gdn_pred_only_avelang_v29_token_split_stage_all_mfma32(
    w: torch.Tensor,
    u: torch.Tensor,
    ...
    *,
    ...
    _qwen_gdn_pred_only_bf16_kernel_v29_token_split_stage_all_mfma32[
...
def _main() -> None:
    ...
    vn = qwen_gdn_pred_only_avelang_v29_token_split_stage_all_mfma32(w, u, initial_state)
    torch.cuda.synchronize()
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_pred_only_avelang_v29_token_split_stage_all_mfma32(w, u, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} token_split_stage_all_v29_pred_only_ms={latency:.6f}")
...
__all__ = [
    ...
    "GRID_SIZE",
    "_qwen_gdn_pred_only_bf16_kernel_v29_token_split_stage_all_mfma32",
    "qwen_gdn_pred_only_avelang_v29_token_split_stage_all_mfma32",
