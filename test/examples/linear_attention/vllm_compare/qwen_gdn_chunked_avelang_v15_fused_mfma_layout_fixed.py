"""Clean Qwen GDN v15 fused MFMA prototype for the Qwen3Next TP4 per-rank target.
...
v15 keeps the v14 standalone MFMA w_u path and adds a fused chunk_gdr+chunk_o
kernel that does not materialize global h/vn buffers.
"""
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_o_bf16_kernel_v15_mfma(
    q_ptr: al.Pointer(al.bf16),
    k_ptr: al.Pointer(al.bf16),
    w_ptr: al.Pointer(al.f32),
    u_ptr: al.Pointer(al.f32),
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        for rep_all in al.range(32):
            ...
            q_scaled_bf16[row_all, col_all] = al.convert(
                al.convert(q[0, token_idx, key_head_idx, col_all], al.f32) * scale,
                al.bf16,
            )
            k_bf16[row_all, col_all] = k[0, token_idx, key_head_idx, col_all]
        ...
        for batch128 in al.range(4):
            k_vec128 = lane_group + batch128 * 4
            a_words128 = w_vec[lane_col, k_vec128]
            b_words128 = state_vec[lane_col, k_vec128]
            a_frag128 = al.view(a_words128, al.Tensor((2, 4, 1), al.bf16))
            b_frag128 = al.view(b_words128, al.Tensor((2, 4, 1), al.bf16))
            pred_acc = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag128[0], b_frag128[0], pred_acc)
            pred_acc = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag128[1], b_frag128[1], pred_acc)
        ...
        for r in al.range(4):
            ...
            global_v = value_base + value_offset
            v_new = u[0, token_idx, value_head_idx, global_v] - pred_acc[r]
            vn_t_bf16[value_offset, token_offset] = al.convert(v_new, al.bf16)
            decay = al.exp(g_last - g[0, token_idx, value_head_idx])
            v_decay_t[value_offset, token_offset] = al.convert(v_new * decay, al.bf16)

        inter_acc = al.full((4,), 0.0, al.f32)
        for batch128_i in al.range(4):
            k_vec128_i = lane_group + batch128_i * 4
            a_words_i0 = q_vec[lane_col, k_vec128_i]
            b_words_i0 = state_vec[lane_col, k_vec128_i]
            a_frag_i0 = al.view(a_words_i0, al.Tensor((2, 4, 1), al.bf16))
            b_frag_i0 = al.view(b_words_i0, al.Tensor((2, 4, 1), al.bf16))
            inter_acc = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_i0[0], b_frag_i0[0], inter_acc)
            inter_acc = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_i0[1], b_frag_i0[1], inter_acc)
        ...
        for r_s in al.range(4):
            token_offset_s = lane_group * 4 + r_s
            source_offset_s = lane_col
            token_idx_s = chunk_start + token_offset_s
            source_idx_s = chunk_start + source_offset_s
            score_value = al.convert(0.0, al.f32)
            if source_offset_s <= token_offset_s:
                decay_s = al.exp(g[0, token_idx_s, value_head_idx] - g[0, source_idx_s, value_head_idx])
                score_value = score_acc[r_s] * decay_s
            score_decay_bf16[token_offset_s, source_offset_s] = al.convert(score_value, al.bf16)

        al.syncthreads()

        intra_acc = al.full((4,), 0.0, al.f32)
        if lane_group == 0:
            a_words_intra = score_vec[lane_col, 0]
            b_words_intra = vn_vec[lane_col, 0]
            a_frag_intra = al.view(a_words_intra, al.Tensor((2, 4, 1), al.bf16))
            b_frag_intra = al.view(b_words_intra, al.Tensor((2, 4, 1), al.bf16))
        ...
        if lane_group == 2:
            a_words_intra = score_vec[lane_col, 1]
            b_words_intra = vn_vec[lane_col, 1]
            a_frag_intra = al.view(a_words_intra, al.Tensor((2, 4, 1), al.bf16))
            b_frag_intra = al.view(b_words_intra, al.Tensor((2, 4, 1), al.bf16))
            intra_acc = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_intra[0], b_frag_intra[0], intra_acc)
        if lane_group == 3:
        ...
        for tile in al.range(8):
            ...
            for r_up in al.range(4):
                vv_up = lane_group * 4 + r_up
                state[vv_up, out_col] = state[vv_up, out_col] * g_last_exp + update_acc[r_up]
    ...
    for rep_final in al.range(16):
        ...
        kk_final = idx_final - vv_final * BK
        global_v_final = value_base + vv_final
        final_state[0, value_head_idx, global_v_final, kk_final] = state[vv_final, kk_final]
        final_state[0, value_head_idx, global_v_final, kk_final + 64] = state[vv_final, kk_final + 64]
...
def _require_v15_qkwug_target_shape(q: torch.Tensor, k: torch.Tensor, w: torch.Tensor, u: torch.Tensor, g: torch.Tensor) -> None:
    if q.dtype != torch.bfloat16 or k.dtype != torch.bfloat16:
        raise ValueError("v15 fused MFMA requires q/k dtype torch.bfloat16.")
    if w.dtype != torch.float32 or u.dtype != torch.float32 or g.dtype != torch.float32:
        raise ValueError("v15 fused MFMA requires w/u/g dtype torch.float32.")
    for name, tensor in (("q", q), ("k", k), ("w", w), ("u", u), ("g", g)):
    ...
    if tuple(w.shape) != (1, num_tokens, 8, 128):
        raise ValueError("w must have shape [1,T,8,128].")
    if tuple(u.shape) != (1, num_tokens, 8, 128):
        raise ValueError("u must have shape [1,T,8,128].")
    if tuple(g.shape) != (1, num_tokens, 8):
        raise ValueError("g must have shape [1,T,8].")


def _validate_initial_state_v15(initial_state: torch.Tensor | None, *, device: torch.device) -> tuple[torch.Tensor | None, bool]:
    has_initial_state = initial_state is not None
    if initial_state is None:
        return None, has_initial_state
    _require_fp32_cuda_contiguous("initial_state", initial_state)
...
def qwen_gdn_fused_chunk_gdr_o_avelang_v15_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    ...
    *,
    ...
    if scale is None:
        scale = 128 ** -0.5
    if chunk_size != BT:
        raise ValueError("v15 fused chunk_gdr_o MFMA only supports chunk_size=16.")
    _require_v15_qkwug_target_shape(q, k, w, u, g)
    batch_size, num_tokens, num_k_heads, num_v_heads, head_dim_k, head_dim_v = _validate_bf16_chunk_gdr_stage(k, w, u, g, chunk_size)
    if (batch_size, num_k_heads, num_v_heads, head_dim_k, head_dim_v) != (1, 4, 8, 128, 128):
        raise ValueError("v15 fused chunk_gdr_o MFMA only supports B=1,Hk=4,Hv=8,K=128,V=128.")
    if num_tokens % BT != 0:
        raise ValueError("v15 fused chunk_gdr_o MFMA requires num_tokens divisible by 16.")
    initial_state_arg, has_initial_state = _validate_initial_state_v15(initial_state, device=q.device)

    num_chunks = _num_chunks(num_tokens, chunk_size)
    output = torch.empty_like(u)
    final_state = torch.empty((1, 8, 128, 128), dtype=torch.float32, device=q.device)
    if initial_state_arg is None:
        initial_state_arg = final_state

    _qwen_gdn_fused_chunk_gdr_o_bf16_kernel_v15_mfma[lambda: ((64, 1, 1), (64, 1, 1))](
        q,
        k,
        w,
        u,
...
def qwen_gdn_chunked_avelang_v15_fused_mfma_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _validate_chunk_size(chunk_size)
    if chunk_size != BT:
        raise ValueError("v15 fused full MFMA only supports chunk_size=16.")
    _validate_bf16_qkvgb(q, k, v, g, beta)
    _require_v14_target_shape(q, k, v, g, beta)
    _validate_initial_state_v15(initial_state, device=q.device)
    if q.shape[1] % BT != 0:
        raise ValueError("v15 fused full MFMA requires num_tokens divisible by 16.")

    g_cumsum = qwen_gdn_chunk_cumsum_avelang_v6_standalone(g, chunk_size=chunk_size)
    a = qwen_gdn_kkt_avelang_v6_standalone(k, g_cumsum, beta, chunk_size=chunk_size, prefer_optimized=True)
    a_solved = qwen_gdn_solve_avelang_v6_standalone(a, chunk_size=chunk_size)
    w, u = qwen_gdn_w_u_avelang_v14_mfma_layout(k, v, g_cumsum, beta, a_solved, chunk_size=chunk_size)
    output, final_state = qwen_gdn_fused_chunk_gdr_o_avelang_v15_mfma_layout(
        q,
        k,
        w,
        u,
    ...
    return g_cumsum, output, a_solved, final_state


def qwen_gdn_chunked_avelang_v15_fused_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if beta is None:
        beta = torch.ones_like(g, dtype=torch.float32, device=g.device).contiguous()
    _, output, _, final_state = qwen_gdn_chunked_avelang_v15_fused_mfma_layout_full(
        q,
        k,
        v,
        g,
