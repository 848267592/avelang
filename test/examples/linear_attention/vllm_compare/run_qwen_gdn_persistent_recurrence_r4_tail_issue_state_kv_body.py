#!/usr/bin/env python3
"""Production-body compiler entry point for the R4-tail state-KV candidate."""
...
def main() -> None:
    ...
    launch, h, v_new, final_state = candidate.run_body(
        k, w, u, g, initial_state, dump_hsaco_dir=args.dump_hsaco_dir)
    for _ in range(args.repeat - 1):
        launch()
    torch.cuda.synchronize()
    print(json.dumps({
        "plan": candidate.PLAN, "T": args.T, "repeat": args.repeat,
        "grid": [candidate.GRID, 1, 1], "workgroup": [candidate.WORKGROUP, 1, 1],
        "emit_audit": False,
