...
_STAGE5C_SOLVE_IMPLS = ("v18", "hierarchical_fp32_v1")


def _solve_bt64_stage5c(a: torch.Tensor, solve_impl: str) -> torch.Tensor:
    """Select the solve implementation explicitly; never silently fall back."""
    if solve_impl == "v18":
        return qwen_gdn_solve_avelang_v18_bt64_layout(a, chunk_size=BT)
    if solve_impl == "hierarchical_fp32_v1":
        return qwen_gdn_solve_bt64_hierarchical_fp32_v1(a)
    raise ValueError(f"solve_impl must be one of {_STAGE5C_SOLVE_IMPLS}, got {solve_impl!r}.")
...
def qwen_gdn_full_bt64_stage4_all_s0_stages(
    ...
    *,
    ...
    """Stage 4 graph with KKT-S0, WU-S1, chunk-o-S0, and an explicit solve choice."""
    h0 = _require_target(q, k, v, g, beta, initial_state)
    if scale is None:
        scale = K_DIM ** -0.5
    g_cumsum = qwen_gdn_chunk_cumsum_avelang_v6_standalone(g, chunk_size=BT)
