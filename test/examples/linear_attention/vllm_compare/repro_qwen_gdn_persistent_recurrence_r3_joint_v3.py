#!/usr/bin/env python3
"""R3 joint_v3: full typed W/K packet-to-dot recurrence pipeline.
...
@avelang.jit
def _qwen_gdn_persistent_recurrence_r3_joint_v3_kernel(
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # R3 preserves R2's interleaved next-chunk issue. The joint_v3 plan
...
def _maybe_dump_hsaco(launch: Callable[[], None]) -> None:
    ...
    target = _HSACO_DUMP_DIR / f"{_qwen_gdn_persistent_recurrence_r3_joint_v3_kernel.fn.__name__}.hsaco"
    if target.exists():
        launch()
        return
    ...
    def wrapped_compile(self, src, target_info, options=None):
        ...
        if not dumped and "persistent_recurrence_r3_joint_v3_kernel" in src.fn.fn.__name__:
    ...
    if not dumped:
        raise RuntimeError("R3 joint_v3 HSACO dump requested, but no matching kernel compiled")
...
def _run_kernel(
    ...
    p1._set_c0_lowering()
    os.environ["AVELANG_PERSISTENT_RECURRENCE_LOWERING"] = "gfx942_bt64_bv32_joint_v3"
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r3_joint_v3_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "rocprof_outputs/qwen_persistent_recurrence_r3_joint_v3",
    )
    ...
    (args.out_dir / "r3_joint_v3_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
    ...
    if not all(bool(result["pass"]) for result in results):
        raise SystemExit("R3 correctness gate failed; do not run resource or performance collection.")
