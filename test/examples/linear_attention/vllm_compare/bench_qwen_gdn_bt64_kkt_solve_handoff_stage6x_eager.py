#!/usr/bin/env python3
"""Williams-block Eager public-API benchmark for Stage 6X KKT-to-solve.

The benchmark deliberately times complete uncaptured public calls. Compilation
and first allocation are warmed before timing; every measured block runs all
six balanced three-way orders of W1, X2 and native vLLM. HIP event and wall
clock are both recorded, while pairing is always at the complete-block level.
"""
...
def calls_for_t(t: int):
    q, k, v, g, beta, h0 = make_inputs(t, 2026072800 + t, "random", True)
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=2026072817)
    ...
    for session in range(args.sessions):
        ...
        for t in t_order:
            calls = calls_for_t(t)
            # Explicit public-path compile/module-load/allocation warmup.
            ...
            for block in range(args.blocks):
                ...
                for reference in ("w1", "vllm"):
                    block_rows.append({
                        "T": t, "chunks": t // 64, "session": session, "block": block,
                        "candidate": "x2", "reference": reference,
                        "event_gain_us": 1000 * (event_medians[reference] - event_medians["x2"]),
                        "wall_gain_us": 1000 * (wall_medians[reference] - wall_medians["x2"]),
                        "x2_event_ms": event_medians["x2"], "reference_event_ms": event_medians[reference],
                    })
    ...
    for t in args.T:
        for reference in ("w1", "vllm"):
            ...
            pairs.append({
                "T": t, "chunks": t // 64, "candidate": "x2", "reference": reference,
