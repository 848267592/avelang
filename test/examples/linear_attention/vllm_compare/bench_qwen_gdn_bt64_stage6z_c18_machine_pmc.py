#!/usr/bin/env python3
"""T=2048 C18 machine-PMC worker.

This is deliberately a profiler worker, not a latency ranking harness.  It
uses the caller-owned C18 output and leaves the compiler/debug environment to
the invoking command so the captured code object is unambiguous.
"""
...
from qwen_gdn_bt64_native_chunko_stage6z_c18_full_physical_region import (
...
def main() -> None:
    ...
    if not bool(torch.isfinite(output).all().item()):
        raise RuntimeError("C18 produced non-finite output")
    print(json.dumps({
        "arm": "c18_full_physical_region",
        "T": args.T,
        "chunks": chunks,
        "workgroup": 256,
        "warmup": args.warmup,
