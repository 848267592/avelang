"""Triton-structured direct-K64 recurrence-update repro for gfx942.
...
The source stages only the immediate 32x32 dot operands; this is the closest
AveLang expression available today to Triton's load-then-block-dot structure.
...
@avelang.jit
def _qwen_gdn_direct_k64_update_current_abi_kernel(
    ...
    # A is V-decay [V32, token32], independently staged for both waves.
    # B is a direct K [K32, token32] block. They are the immediate operands of
    # the next MFMA only; no broad K transpose/view survives across phases.
    a_stage = al.make_shared((2, 32, 32), al.bf16)
    b_stage = al.make_shared((32, 32), al.bf16)
    a_vec = al.view(a_stage, al.i32, al.make_layout((2, 32, 4, 4), (32 * 4 * 4, 4 * 4, 4, 1)))
    b_vec = al.view(b_stage, al.i32, al.make_layout((32, 4, 4), (4 * 4, 4, 1)))
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # The four output tiles are issued serially by each wave. This keeps
        # WORKGROUP=128 while retaining the complete [V64,K128] state in AGPRs.
        for k_half in al.range(2):
            for col_half in al.range(2):
                update_acc = al.full((16,), 0.0, al.f32)
                for token_half in al.range(2):
                    # All 128 threads load one immediate A/B operand tile.
                    for rep_a in al.range(16):
                        linear_a = tid + rep_a * WORKGROUP
                        a_wave = linear_a // (32 * 32)
                        a_rem = linear_a - a_wave * (32 * 32)
                        a_row = a_rem // 32
                        a_tok = a_rem - a_row * 32
                        token_idx = chunk_start + token_half * 32 + a_tok
                        global_v = value_base + a_wave * 32 + a_row
                    ...
                    for rep_b in al.range(8):
                        linear_b = tid + rep_b * WORKGROUP
                        b_row = linear_b // 32
                        b_tok = linear_b - b_row * 32
                        b_stage[b_row, b_tok] = k[
                            0,
                            chunk_start + token_half * 32 + b_tok,
                            key_head_idx,
                            k_half * 64 + col_half * 32 + b_row,
                        ]

                    al.syncthreads()

                    for k_pack in al.range(2):
                        word = k_pack * 2 + lane_group
                        a_words = a_vec[wave_id, lane_col, word]
                        b_words = b_vec[lane_col, word]
                ...
                if k_half == 0:
                    if col_half == 0:
                        for acc_i in al.range(16):
                            h1_lo[acc_i] = h1_lo[acc_i] * g_last_exp + update_acc[acc_i]
                    else:
                        for acc_i in al.range(16):
                            h1_hi[acc_i] = h1_hi[acc_i] * g_last_exp + update_acc[acc_i]
                else:
                    if col_half == 0:
                        for acc_i in al.range(16):
                            h2_lo[acc_i] = h2_lo[acc_i] * g_last_exp + update_acc[acc_i]
                    else:
                        for acc_i in al.range(16):
                            h2_hi[acc_i] = h2_hi[acc_i] * g_last_exp + update_acc[acc_i]
...
def qwen_gdn_direct_k64_update_current_abi(
    k: torch.Tensor,
    v_new: torch.Tensor,
    g: torch.Tensor,
    ...
    def launch() -> None:
        _qwen_gdn_direct_k64_update_current_abi_kernel[lambda: ((H_V * 2, 1, 1), (WORKGROUP, 1, 1))](
            k,
            v_new,
            g,
            initial_state,
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_direct_k64_update_current_abi_kernel")
    launch()
...
def run_case(t: int, *, seed: int, warmup: int, repeat: int, check: bool) -> dict[str, float | int | bool]:
    ...
    def launch() -> tuple[torch.Tensor, torch.Tensor]:
        return qwen_gdn_direct_k64_update_current_abi(k, v_new, g, initial)
