#!/usr/bin/env python3
"""Compile-only exact-LTO/MIR/ISA capture for one Z6G arm.

The constexpr g mode is captured in a separate process/output directory so
the stable and ideal code objects cannot be conflated by the same kernel name.
No kernel launch, benchmark, selector or X2 graph is executed here.
"""
...
def make_ast_source(t: int, mode: int):
    if t < 64 or t % 64:
    ...
    bound, specialization, options = binder(
        q, k, vn, h, g, out, float(128 ** -0.5), t, chunks, mode,
    )
...
def main() -> None:
    ...
    mode = Z6G_STABLE if args.arm == "z6g_s" else Z6G_IDEAL
    if args.out_dir is None:
        args.out_dir = COMPILE_BUG / f"codex_qwen_bt64_stage6z_{args.arm}_machine"
    ...
    kernel = _qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_z6g
    print(f"[{args.arm}-machine] reconstruct ASTSource T={args.T}", flush=True)
    src, target, options = make_ast_source(args.T, mode)
    ...
    if args.skip_initial_mlir:
        ...
        try:
            print(f"[{args.arm}-machine] dumping initial MLIR", flush=True)
            path = out / "initial_mlir.mlir"
            path.write_text(build_generator(src).get_mlir())
            summary["initial_mlir"] = str(path)
        except Exception as exc:
            path = out / "initial_mlir.status.txt"
    ...
    print(f"[{args.arm}-machine] lowering LLVM IR", flush=True)
    llvm_path.write_text(build_generator(src).get_llvm_ir(target.tuple, target.chip, 4, options.num_warps))
    summary["llvm"] = str(llvm_path)
    if args.skip_pre_lto_assembly:
        summary["pre_lto_assembly"] = "skipped by command line"
    else:
        print(f"[{args.arm}-machine] generating pre-LTO AMDGCN assembly", flush=True)
        assembly_path = out / "pre_lto_amdgcn.s"
        assembly_path.write_text(build_generator(src).get_assembly(target.tuple, target.chip, 4, options.num_warps))
        summary["pre_lto_assembly"] = str(assembly_path)

    print(f"[{args.arm}-machine] linking HSACO", flush=True)
    binary = build_generator(src).compile_to_binary_bytes(target.tuple, target.chip, 4, options.num_warps)
    hsaco = out / f"{args.arm}_fixed.hsaco"
    ...
    if argv_files:
        print(f"[{args.arm}-machine] replaying exact LTO", flush=True)
        exact_lto.mkdir(parents=True, exist_ok=True)
        result = subprocess.run(
            [sys.executable, str(replay), "--argv-file", str(argv_files[0]), "--out-dir", str(exact_lto), "--kernel", kernel.fn.__name__],
    ...
    print(f"[{args.arm}-machine] dumping llc stop-point MIR", flush=True)
