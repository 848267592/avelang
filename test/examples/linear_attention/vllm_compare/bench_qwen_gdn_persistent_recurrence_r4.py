#!/usr/bin/env python3
"""Fresh-process R4 recurrence-body diagnostic against native and Triton controls."""
...
def _worker(t: int, *, seed: int, warmup: int, repeat: int, session: int) -> dict[str, Any]:
    ...
    if not all(all(bool(value["finite"]) for value in check.values()) for check in checks.values()):
        raise RuntimeError("non-finite output in R4 body comparison")
    for call in calls:
        for _ in range(warmup):
            call.launch()
    torch.cuda.synchronize()
    base_order = [0, 1, 2, 3, 4, 5, 6, 6, 5, 4, 3, 2, 1, 0]
...
def _parent(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "contract": {
            ...
            "order": "rotating palindromic seven-arm",
