...
def _build_wu_inputs(k, v, g, beta, chunk_size: int):
...
def _assert_close(name: str, actual: torch.Tensor, expected: torch.Tensor, atol: float, rtol: float):
    _, max_abs, max_rel = _err(actual, expected)
...
def test_v14_w_u_mfma_matches_v6_w_u(t: int):
    chunk = 16
    _, k, v, g, beta, _ = _make_inputs(t, False, seed=7000 + t)
    ...
    w_new, u_new = qwen_gdn_w_u_avelang_v14_mfma_layout(k, v, g_cumsum, beta, a_solved, chunk_size=chunk)

    try:
        _assert_close("w", w_new, w_ref, atol=1e-3, rtol=1e-3)
        _assert_close("u", u_new, u_ref, atol=1e-3, rtol=1e-3)
    except AssertionError:
        w_fb, u_fb = qwen_gdn_w_u_avelang_v6_standalone(
            k,
            v,
            g_cumsum,
            beta,
        ...
        _assert_close("v6_opt_vs_fallback_w", w_ref, w_fb, atol=1e-5, rtol=1e-5)
        _assert_close("v6_opt_vs_fallback_u", u_ref, u_fb, atol=1e-5, rtol=1e-5)
        raise


@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v14_full_forward_matches_v12(t: int, with_initial_state: bool):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=8000 + t + int(with_initial_state))
    scale = 128 ** -0.5
    ...
    out_new, fs_new = qwen_gdn_chunked_avelang_v14_mfma_layout(
        q,
        k,
        v,
        g,
