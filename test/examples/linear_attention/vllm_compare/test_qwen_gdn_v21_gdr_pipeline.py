...
@pytest.mark.parametrize("variant", ["w", "wk"])
@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v21_pipeline_chunk_gdr_matches_v17_predecay(t: int, with_initial_state: bool, variant: str):
    chunk = 16
    _, k, v, g, beta, initial_state = _make_inputs(
        t,
        with_initial_state,
        seed=21000 + t + int(with_initial_state) * 17,
    )
    g_cumsum, w, u, gdr_decay, gdr_g_last_exp = _build_gdr_inputs(k, v, g, beta, chunk)

    h_ref, vn_ref, fs_ref = qwen_gdn_chunk_gdr_avelang_v17_4wave_predecay_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    h_new, vn_new, fs_new = qwen_gdn_chunk_gdr_avelang_v21_4wave_pipeline_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    _assert_close(f"h_{variant}", h_new, h_ref)
    _assert_close(f"vn_{variant}", vn_new, vn_ref)
    _assert_close(f"final_state_{variant}", fs_new, fs_ref)


@pytest.mark.parametrize("variant", ["w", "wk"])
@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v21_pipeline_full_forward_matches_v17_predecay(t: int, with_initial_state: bool, variant: str):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(
        t,
        with_initial_state,
        seed=22000 + t + int(with_initial_state) * 19,
    )
    scale = 128 ** -0.5

    out_ref, fs_ref = qwen_gdn_chunked_avelang_v17_predecay_mfma_layout(
        q,
        k,
        v,
        g,
    ...
    out_new, fs_new = qwen_gdn_chunked_avelang_v21_gdr_pipeline_layout(
        q,
        k,
        v,
        g,
    ...
    _assert_close(f"output_{variant}", out_new, out_ref)
    _assert_close(f"final_state_full_{variant}", fs_new, fs_ref)
