...
@pytest.mark.parametrize("variant", ["full_baseline", "distributed_vn_vdecay"])
@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v22_correct_variants_match_v17_predecay(t: int, with_initial_state: bool, variant: str):
    chunk = 16
    k, v, g, beta, initial_state = _make_inputs(
        t,
        with_initial_state,
        seed=22000 + t + int(with_initial_state) * 31,
    )
    ...
    h_new, vn_new, fs_new = qwen_gdn_chunk_gdr_avelang_v22_ablation_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
...
def test_v22_ablation_variants_smoke(variant: str):
    assert variant in VARIANT_TO_MODE
    chunk = 16
    k, v, g, beta, initial_state = _make_inputs(32, True, seed=23100 + VARIANT_TO_MODE[variant])
    g_cumsum, w, u, gdr_decay, gdr_g_last_exp = _build_gdr_inputs(k, v, g, beta, chunk)
    h, vn, final_state = qwen_gdn_chunk_gdr_avelang_v22_ablation_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    torch.cuda.synchronize()
    print(f"{variant}_shapes,h={tuple(h.shape)},vn={tuple(vn.shape)},final_state={tuple(final_state.shape)}")
