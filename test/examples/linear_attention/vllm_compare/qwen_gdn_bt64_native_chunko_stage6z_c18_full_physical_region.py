"""Stage 6Z C18: compiler-owned full physical-region logical block-dot.

This experimental-only source keeps the validated Q/H/K schedule and replaces
the handwritten Phase-C V producer/consumer with the same generic logical
block-dot operation used by H and K.  C18 owns Q/H/K/V producer, shared slot,
and physical MFMA consumer construction in one compiler-internal plan.
...
@avelang.jit
def _qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_c18_full_physical_region(
    ...
    # C18 routes V through the existing logical block-dot contract.  Passing
    # vn in both source slots is the compiler-internal role marker; the C18
    # physical-region planner emits one V BF16x8 producer and the score@V
    # packed MFMA consumer.  No source-level scalar V staging remains.
...
def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c18_full_physical_region_launch_into(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    *,
    ...
    if WORKGROUP != BDV2_WORKGROUP_CONTRACT:
        raise RuntimeError("C18 shape contract violation: WG must remain 256")
    set_block_dot_lowering(lowering)
    set_block_dot_planner(planner)
    set_block_dot_operand_preservation(preservation)
    os.environ["AVELANG_STAGE6Z_FULL_PHYSICAL_REGION"] = "c18"
    ...
    if scale is None:
        scale = K_DIM ** -0.5
    _qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_c18_full_physical_region[
        lambda: ((num_chunks * H_V * 2, 1, 1), (WORKGROUP, 1, 1))
    ](q, k, v_new_bf16, h_bf16, g, output_bf16, float(scale), t, num_chunks)


def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c18_full_physical_region(
    q: torch.Tensor,
    k: torch.Tensor,
    v_new_bf16: torch.Tensor,
    h_bf16: torch.Tensor,
...
__all__ = [
    "_qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_c18_full_physical_region",
    "qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c18_full_physical_region",
    "qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c18_full_physical_region_launch_into",
]
