"""Stage 6Z C19: compiler-owned full physical-region logical block-dot.

This experimental-only source presents one shared arena and lets the C19
FullPhysicalRegionPlan generate all Q/H/K/V producer and consumer ownership.
The same logical block-dot operation is used by Q, H, K and V; C19 owns the
physical producer, shared slot, consumer and ownership assertions.
...
@avelang.jit
def _qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_c19_full_physical_region(
    ...
    # C19 exposes one shared arena to the compiler-owned Q/H/K/V plan.
    # The source does not produce Q/H/K/V packets or construct a phase vector.
    shared = al.make_shared((C19_SHARED_ROWS, BK), al.bf16)
    phase = shared
    zero_f32 = al.convert(0.0, al.f32)
    scale_f32 = al.convert(scale, al.f32)
    q_owner_acc = al.full((16,), 0.0, al.f32)
    _q_owner = al.amdgpu.block_dot_bf16_f32_logical(
        shared,
        shared,
        q,
        q,
    ...
    # The Q cache must remain intact until *both* source-half K products have
    # consumed it.  Writing score-half zero into rows 0..127 immediately
    # after its K loop would overwrite Q rows that source-half one still
    # reads.  C19 therefore keeps two score accumulators, completes both K
    # phases first, and only then reuses the Q band for score/V storage.
    ...
    if value_half == 0:
        for r in al.range(16):
            ...
            # Both Q/K consumers are now finished, so rows 0..127 are safe
            # to reuse for score half zero before score@V.
        ...
        for r in al.range(16):
            ...
            # Both Q/K consumers are now finished, so rows 0..127 are safe
            # to reuse for score half one before score@V.
            phase[token_offset * 2 + 1, out_col] = al.convert(score, al.bf16)
    al.syncthreads()

    # V is a logical source; C19 owns its producer and score@V consumer.
...
def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c19_full_physical_region_launch_into(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    *,
    ...
    if WORKGROUP != BDV2_WORKGROUP_CONTRACT:
        raise RuntimeError("C19 shape contract violation: WG must remain 256")
    set_block_dot_lowering(lowering)
    set_block_dot_planner(planner)
    set_block_dot_operand_preservation(preservation)
    os.environ["AVELANG_STAGE6Z_FULL_PHYSICAL_REGION"] = "c19"
...
def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c19_full_physical_region(
    q: torch.Tensor,
    k: torch.Tensor,
    v_new_bf16: torch.Tensor,
    h_bf16: torch.Tensor,
...
__all__ = [
    "_qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_c19_full_physical_region",
    "qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c19_full_physical_region",
    "qwen_gdn_chunk_o_bt64_native_chunko_stage6z_c19_full_physical_region_launch_into",
]
