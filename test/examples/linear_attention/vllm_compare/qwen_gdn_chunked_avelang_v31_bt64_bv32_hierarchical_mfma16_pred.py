"""Qwen GDN v31 MFMA16-pred hierarchical BT64/BV32 chunk_gdr experiment.

This file is intentionally isolated from the production v23/v24 baselines.

The kernel is a scheduling experiment for the v31 MFMA16-pred path:
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v31_bt64_bv32_hierarchical_mfma16_pred(
...
def _validate_inputs(
    ...
    if chunk_size != BT:
        raise ValueError("v31 MFMA16-pred hierarchical chunk_gdr only supports chunk_size=64.")
    ...
    if num_tokens % BT != 0:
        raise ValueError("v31 MFMA16-pred hierarchical chunk_gdr requires T divisible by 64.")
...
def qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v31_bt64_bv32_hierarchical_mfma16_pred[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v31_bt64_bv32_hierarchical_mfma16_pred")
    launch()
...
def _smoke_main() -> None:
    ...
    h, final_state = qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred(
        k,
        w,
        u,
        gdr_decay,
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred(k, w, u, gdr_decay, gdr_g_last_exp, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v31_bt64_bv32_hierarchical_mfma16_pred_ms={latency:.6f}")
