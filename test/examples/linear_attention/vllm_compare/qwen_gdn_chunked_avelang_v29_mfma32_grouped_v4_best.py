"""Qwen GDN v29 pred-only BT64/BV32 MFMA32 grouped-v4 best variant.
...
Important
---------
This clean experiment archives the best correctness-preserving v29 pred-only
source variant so far: grouped contiguous-V scalar epilogue stores. It keeps a
baseline mode only for local A/B comparison and removes profiling-only public
variants.
...
@avelang.jit
def _qwen_gdn_pred_only_bf16_kernel_v29_mfma32_grouped_v4_best(
...
def _variant_flags(variant: str) -> dict[str, bool]:
    if variant not in _VARIANTS:
        raise ValueError(f"unknown v29 grouped_v4_best variant {variant!r}; expected one of {sorted(_VARIANTS)}")
    return _VARIANTS[variant]


def qwen_gdn_pred_only_avelang_v29_mfma32_grouped_v4_best(
    w: torch.Tensor,
    u: torch.Tensor,
    initial_state: torch.Tensor | None = None,
    *,
    chunk_size: int = BT,
    variant: str = MODE_GROUPED_V4_EPILOGUE,
) -> torch.Tensor:
    ...
    def launch() -> None:
        _qwen_gdn_pred_only_bf16_kernel_v29_mfma32_grouped_v4_best[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_pred_only_bf16_kernel_v29_mfma32_grouped_v4_best")
    launch()
...
def qwen_gdn_pred_only_avelang_v29_mfma32(
    ...
    *,
    ...
    return qwen_gdn_pred_only_avelang_v29_mfma32_grouped_v4_best(
        w,
        u,
        initial_state,
...
def _smoke_main() -> None:
    ...
    parser.add_argument("--variant", choices=sorted(_VARIANTS), default=MODE_GROUPED_V4_EPILOGUE)
    ...
    vn = qwen_gdn_pred_only_avelang_v29_mfma32_grouped_v4_best(w, u, initial_state, variant=args.variant)
    torch.cuda.synchronize()
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_pred_only_avelang_v29_mfma32_grouped_v4_best(w, u, initial_state, variant=args.variant),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} variant={args.variant} v29_grouped_v4_best_ms={latency:.6f}")
