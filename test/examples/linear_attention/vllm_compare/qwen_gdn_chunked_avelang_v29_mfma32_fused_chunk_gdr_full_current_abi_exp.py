"""Experimental v29 compact-K recurrence with the current-vLLM BF16 ABI.
...
This is an ABI-alignment experiment, not a production recurrence.  It keeps
the compact-K schedule, shared-memory allocation, MFMA sequence, and launch
unchanged from the overflowing K-fragment rewrite.  Only the recurrence
boundary is changed to the current-vLLM contract:
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_full_current_abi_exp_bf16_kernel_v29_mfma32(
...
def _validate_inputs(
    ...
    if w.dtype != torch.bfloat16 or u.dtype != torch.bfloat16:
        raise ValueError("current-ABI compact-K requires BF16 w/u.")
...
def qwen_gdn_fused_chunk_gdr_full_current_abi_avelang_v29_mfma32(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
    ...
    *,
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_chunk_gdr_full_current_abi_exp_bf16_kernel_v29_mfma32")
    launch()
    return h, v_new, final_state


def qwen_gdn_fused_chunk_gdr_full_current_abi_reference(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
...
def _smoke_main() -> None:
    ...
    h, v_new, final_state = qwen_gdn_fused_chunk_gdr_full_current_abi_avelang_v29_mfma32(
        k,
        w,
        u,
        gdr_decay,
    ...
    if not args.no_check_ref:
        h_ref, v_new_ref, final_ref = qwen_gdn_fused_chunk_gdr_full_current_abi_reference(
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_chunk_gdr_full_current_abi_avelang_v29_mfma32(
            k, w, u, gdr_decay, gdr_g_last_exp, initial_state
        ),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v29_compact_k_current_abi_chunk_gdr_ms={latency:.6f}")
