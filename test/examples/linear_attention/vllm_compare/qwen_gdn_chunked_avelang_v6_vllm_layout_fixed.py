...
内联了 BF16 q/k/v 读取后以 FP32 累加的
本文件没有新增性能优化，只做依赖消除和函数改名；数学行为、dtype 支持范围与
shape 支持范围保持和现有 v6 一致。本文件是整合前面零碎的代码片段的集成
"""
...
def _qwen_gdn_chunk_gdr_fp32_v6_standalone(
    ...
    *,
    ...
    """根据 chunk 入口 state 生成 h、vn 和 final_state；state layout 对齐 vLLM: [B, Hv, V, K]。"""
    _validate_chunk_size(chunk_size)
...
def _qwen_gdn_chunk_o_fallback_v6_standalone(
    ...
    *,
    ...
    """根据 chunk state 和 chunk 内 vn 生成最终 output；h layout 对齐 vLLM: [B, C, Hv, V, K]。"""
    _validate_chunk_size(chunk_size)
    ...
    if num_v_heads % num_k_heads != 0:
        raise ValueError(f"Hv must be divisible by Hk, got Hv={num_v_heads}, Hk={num_k_heads}.")
    num_chunks = _num_chunks(num_tokens, chunk_size)
    expected_h_shape = (batch_size, num_chunks, num_v_heads, head_dim_v, head_dim_k)
    ...
    if al.thread_id(0) == 0:
        if program_id < total_programs:
            ...
            for vv in al.range(head_dim_v):
                inter = al.convert(0.0, al.f32)
                for kk in al.range(head_dim_k):
                    inter = inter + q_scaled[kk] * q_decay * h[batch_idx, chunk_idx, value_head_idx, vv, kk]
...
def qwen_gdn_chunked_avelang_v6_standalone(
    ...
    *,
    ...
    """运行完整 standalone v6 forward，支持统一 FP32 或统一 BF16 q/k/v 输入；state layout 对齐 vLLM: [B, Hv, V, K]。"""
...
def qwen_gdn_chunked_avelang_v6_vllm_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    """vLLM-style thin entry for operator-level comparison.

    Input layout follows vLLM/FLA:
        q: [B, T, H, K]
        k: [B, T, H, K]
        v: [B, T, HV, V]
        g: [B, T, HV]
    ...
    This function intentionally does not transpose state tensors. It is only a
    thin API adapter around the vLLM-layout implementation in this file.
    """
    if beta is None:
        beta = torch.ones_like(g, dtype=torch.float32, device=g.device).contiguous()
    _, output, _, _, final_state = qwen_gdn_chunked_avelang_v6_standalone(
        q,
        k,
        v,
        g,
...
def qwen_gdn_chunk_gated_delta_rule_vllm_compatible(
    ...
    *,
    ...
    """vLLM ChunkGatedDeltaRule-compatible surface for equal-length benchmark.

    This mirrors the vLLM prefill API shape, but this minimal v6 layout patch
    still supports only equal-length, head_first=False inputs. For a fair
    performance benchmark against vLLM with use_qk_l2norm_in_kernel=True, add
    native q/k L2-normalization in Avelang kernels or pre-normalize q/k outside
    both implementations and pass use_qk_l2norm_in_kernel=False.
    """
    if cu_seqlens is not None:
        raise NotImplementedError("cu_seqlens / packed variable-length input is not supported in this minimal vLLM-layout patch.")
    if head_first:
        raise NotImplementedError("head_first=True is not supported; expected vLLM layout [B, T, H, D].")
    if use_qk_l2norm_in_kernel:
        raise NotImplementedError("Native in-kernel q/k L2 norm is not implemented in this minimal patch yet.")
    if not output_final_state:
        raise NotImplementedError("output_final_state=False is not supported; this implementation always returns final_state.")
    return qwen_gdn_chunked_avelang_v6_vllm_layout(
        q,
        k,
        v,
        g,
