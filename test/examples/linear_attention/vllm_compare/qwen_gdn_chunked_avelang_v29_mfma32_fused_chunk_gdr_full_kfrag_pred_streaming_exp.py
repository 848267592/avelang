"""Experimental v29 full chunk_gdr with compiler K-fragment rewrite plus pred streaming.
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_full_kfrag_pred_streaming_exp_bf16_kernel_v29_mfma32(
    ...
    # Lane-major accumulator serialization has the same 8 KiB footprint as
    # pred_partial[2, 32, 32], but avoids the producer-side permutation while
    # the 16-element MFMA32 result is still live.
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        for token_tile in al.range(2):
            ...
            for rep_corr in al.range(8):
                ...
                token_hi = token_off // 8
                value_hi = local_v // 8
                value_low = local_v - value_hi * 8
                lane_for_pred = (value_low & 3) * 16 + ((value_low // 4) * 8) + (token_off & 7)
                acc_for_pred = value_hi * 4 + token_hi
                pred_value = (
                    pred_acc_serial[0, lane_for_pred, acc_for_pred]
                    + pred_acc_serial[1, lane_for_pred, acc_for_pred]
                )
            ...
            for local_tile_u in al.range(4):
                ...
                for token_sub in al.range(4):
                    ...
                    if lane_group == 0:
                        ...
                        b_frag_u16 = al.amdgpu.qwen_update_kfrag_load_bf16x4(
                            k_all_t, k, tid, key_head_idx, chunk_start,
                            base_k_u + lane_col, chunk_start + token_sub * 16,
                        )
                        a_frag_u16 = al.view(a_words_u16, al.Tensor((2, 4, 1), al.bf16))
                        update_acc16 = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_u16[0], b_frag_u16, update_acc16)
                    if lane_group == 1:
                        a_words_u16 = vdecay_vec[v_half * 16 + lane_col, pack_base]
                        b_frag_u16 = al.amdgpu.qwen_update_kfrag_load_bf16x4(
                            k_all_t, k, tid, key_head_idx, chunk_start,
                            base_k_u + lane_col, chunk_start + token_sub * 16 + 4,
                        )
                        a_frag_u16 = al.view(a_words_u16, al.Tensor((2, 4, 1), al.bf16))
                        update_acc16 = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_u16[1], b_frag_u16, update_acc16)
                    if lane_group == 2:
                        a_words_u16 = vdecay_vec[v_half * 16 + lane_col, pack_base + 1]
                        b_frag_u16 = al.amdgpu.qwen_update_kfrag_load_bf16x4(
                            k_all_t, k, tid, key_head_idx, chunk_start,
                            base_k_u + lane_col, chunk_start + token_sub * 16 + 8,
                        )
                        a_frag_u16 = al.view(a_words_u16, al.Tensor((2, 4, 1), al.bf16))
                        update_acc16 = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_u16[0], b_frag_u16, update_acc16)
                    if lane_group == 3:
                        ...
                        b_frag_u16 = al.amdgpu.qwen_update_kfrag_load_bf16x4(
                            k_all_t, k, tid, key_head_idx, chunk_start,
                            base_k_u + lane_col, chunk_start + token_sub * 16 + 12,
                        )
                        a_frag_u16 = al.view(a_words_u16, al.Tensor((2, 4, 1), al.bf16))
                        update_acc16 = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_u16[1], b_frag_u16, update_acc16)
...
def qwen_gdn_fused_chunk_gdr_full_kfrag_pred_streaming_avelang_v29_mfma32(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_fused_chunk_gdr_full_kfrag_pred_streaming_exp_bf16_kernel_v29_mfma32[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_chunk_gdr_full_kfrag_pred_streaming_exp_bf16_kernel_v29_mfma32")
    launch()
...
def _smoke_main() -> None:
    ...
    h, final_state = qwen_gdn_fused_chunk_gdr_full_kfrag_pred_streaming_avelang_v29_mfma32(
        k,
        w,
        u,
        gdr_decay,
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_chunk_gdr_full_kfrag_pred_streaming_avelang_v29_mfma32(k, w, u, gdr_decay, gdr_g_last_exp, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v29_kfrag_pred_streaming_exp_chunk_gdr_ms={latency:.6f}")
