"""Qwen GDN v29 pred-only BT64/BV32 MFMA32 experiment.
...
Important
---------
This file intentionally implements pred-only first. It does not update state,
does not store h, and does not write final_state.

The main correctness risk is the 32x32 MFMA accumulator fragment layout. The
candidate mapping used below is a common 32x32-style mapping, but Codex should
verify it against a one-hot MFMA32 layout probe and fix it if needed.
...
@avelang.jit
def _qwen_gdn_pred_only_bf16_kernel_v29_mfma32(
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        for token_tile in al.range(2):
            ...
            # Store raw accumulator fragment into pred_partial using a
            # candidate 32x32 accumulator layout.
            for acc_i in al.range(16):
                # Candidate 32x32 layout:
                #
                # row: 4 groups of 8 rows
                # col: 4 groups of 8 columns, with lane_group selecting
                #      one of four columns inside a 4-column sub-block.
                #
                # This covers 32x32 exactly over 64 lanes * 16 acc values.
            ...
            # ----------------------------------------------------------
            # Reduce wave partials and store vn = u - pred.
...
def qwen_gdn_pred_only_avelang_v29_mfma32(
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_pred_only_bf16_kernel_v29_mfma32[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_pred_only_bf16_kernel_v29_mfma32")
    launch()
...
def _smoke_main() -> None:
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_pred_only_avelang_v29_mfma32(w, u, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v29_pred_only_ms={latency:.6f}")
