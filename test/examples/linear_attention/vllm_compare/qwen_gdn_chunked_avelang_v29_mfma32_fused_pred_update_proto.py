"""Qwen GDN v29 MFMA32 fused pred-update prototype.
...
Important
---------
This experiment starts from the original v29 k-split MFMA32 pred schedule and
tests the next big-fix idea: avoid global VN materialization by consuming
`u - pred` directly in an update-friendly shared layout.

Only the skeleton path is implemented in this file. It computes pred, computes
corrected u, stages it as BF16 `[V, token]` shared data, and emits one tiny side
effect. It is profiling-only and is not a full chunk_gdr implementation.
...
MODE_GROUPED_V4_EPILOGUE = "grouped_v4_epilogue"
MODE_FUSED_PRED_UPDATE_SKELETON = "fused_pred_update_skeleton"

EPILOGUE_BASELINE = 0
EPILOGUE_GROUPED_V4 = 2
EPILOGUE_FUSED_PRED_UPDATE_SKELETON = 3
...
_VARIANTS = {
    MODE_BASELINE: dict(
        ...
        epilogue_kind=EPILOGUE_BASELINE,
    ),
    MODE_GROUPED_V4_EPILOGUE: dict(
        do_acc_unpack=True,
        do_reduce=True,
        store_full_vn=True,
        constant_state=False,
        precomputed_mapping=False,
        epilogue_kind=EPILOGUE_GROUPED_V4,
    ),
    # Profiling-only fused skeleton. It stages all corrected values to shared
    # [V, token] layout and avoids full global VN materialization.
    MODE_FUSED_PRED_UPDATE_SKELETON: dict(
        do_acc_unpack=True,
        do_reduce=True,
        store_full_vn=False,
        constant_state=False,
        precomputed_mapping=False,
        epilogue_kind=EPILOGUE_FUSED_PRED_UPDATE_SKELETON,
    ),
}


@avelang.jit
def _qwen_gdn_fused_pred_update_skeleton_bf16_kernel_v29_mfma32(
    ...
    u_corr_t_bf16 = al.make_shared((BV, 32), al.bf16)
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        for token_tile in al.range(2):
            ...
            if do_reduce:
                if store_full_vn:
                    if epilogue_kind == EPILOGUE_GROUPED_V4:
                        # 32 tokens * 8 groups of 4 V values = 256 groups.
                        # 128 threads * 2 reps = 256 grouped scalar stores.
                        for rep4 in al.range(2):
                            ...
                            token_idx4 = chunk_start + token_base + token_off4
                            base_offset = token_idx4 * (8 * 128) + value_head_idx * 128 + value_base + local_v4

                            pred0 = pred_partial[0, token_off4, local_v4] + pred_partial[1, token_off4, local_v4]
                            pred1 = pred_partial[0, token_off4, local_v4 + 1] + pred_partial[1, token_off4, local_v4 + 1]
                            pred2 = pred_partial[0, token_off4, local_v4 + 2] + pred_partial[1, token_off4, local_v4 + 2]
                            pred3 = pred_partial[0, token_off4, local_v4 + 3] + pred_partial[1, token_off4, local_v4 + 3]

                            vn_flat[base_offset] = u_flat[base_offset] - pred0
                            vn_flat[base_offset + 1] = u_flat[base_offset + 1] - pred1
                            vn_flat[base_offset + 2] = u_flat[base_offset + 2] - pred2
                            vn_flat[base_offset + 3] = u_flat[base_offset + 3] - pred3
                    else:
                    ...
                    # Fused pred-update skeleton:
                    # compute all corrected values and stage them in the
                    # V-major/token-minor layout consumed by update MFMA.
                    for rep_skel in al.range(8):
                        linear_skel = tid + rep_skel * WORKGROUP
                        token_off_skel = linear_skel // BV
                        local_v_skel = linear_skel - token_off_skel * BV

                        token_idx_skel = chunk_start + token_base + token_off_skel
                        out_offset_skel = (
                            token_idx_skel * (8 * 128)
                            + value_head_idx * 128
                            + value_base
                            + local_v_skel
                        )
                        pred_skel = (
                            pred_partial[0, token_off_skel, local_v_skel]
                            + pred_partial[1, token_off_skel, local_v_skel]
                        )
                        u_corr_t_bf16[local_v_skel, token_off_skel] = al.convert(
                            u_flat[out_offset_skel] - pred_skel,
                            al.bf16,
                        )
                    ...
                    if tid == 0:
                        side_offset = (chunk_start + token_base) * (8 * 128) + value_head_idx * 128 + value_base
                        vn_flat[side_offset] = al.convert(u_corr_t_bf16[0, 0], al.f32)
            else:
                if do_acc_unpack:
...
def _variant_flags(variant: str) -> dict[str, bool]:
    if variant not in _VARIANTS:
        raise ValueError(f"unknown v29 fused_pred_update_proto variant {variant!r}; expected one of {sorted(_VARIANTS)}")
    return _VARIANTS[variant]


def qwen_gdn_fused_pred_update_proto_avelang_v29_mfma32(
    w: torch.Tensor,
    u: torch.Tensor,
    initial_state: torch.Tensor | None = None,
    *,
    chunk_size: int = BT,
    variant: str = MODE_FUSED_PRED_UPDATE_SKELETON,
) -> torch.Tensor:
    ...
    def launch() -> None:
        _qwen_gdn_fused_pred_update_skeleton_bf16_kernel_v29_mfma32[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_pred_update_skeleton_bf16_kernel_v29_mfma32")
    launch()
...
def qwen_gdn_pred_only_avelang_v29_mfma32(
    ...
    *,
    ...
    return qwen_gdn_fused_pred_update_proto_avelang_v29_mfma32(
        w,
        u,
        initial_state,
...
def _smoke_main() -> None:
    ...
    parser.add_argument("--variant", choices=sorted(_VARIANTS), default=MODE_FUSED_PRED_UPDATE_SKELETON)
    ...
    vn = qwen_gdn_fused_pred_update_proto_avelang_v29_mfma32(w, u, initial_state, variant=args.variant)
    torch.cuda.synchronize()
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_pred_update_proto_avelang_v29_mfma32(w, u, initial_state, variant=args.variant),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} variant={args.variant} v29_fused_pred_update_proto_ms={latency:.6f}")
