#!/usr/bin/env python3
"""B2: B0 plus a one-chunk-ahead, single-LDS-bank software pipeline.

The only experimental variable is explicit next-chunk global-load lookahead:
W is issued before current pred, while U/g/K are issued before current update.
Those values stay in per-thread local storage until current LDS consumers have
finished, then overwrite the same W/K LDS bank for the next BT64 chunk.
"""
...
@avelang.jit
def _qwen_gdn_direct_k64_bv32_full_sequence_b2_lookahead_kernel(
    ...
    # B2 owns one full current W/K bank, never a second LDS bank. Lookahead
    # values reside in per-thread local storage until this bank is dead.
    ...
    g_stage = al.make_shared((BT,), al.f32)
    ...
    # One 64-token W/K block maps to 64 BF16 items per thread. U maps to 16
    # items per thread for the CTA's V32 rows. These are the register-backed
    # lookahead window; they are not a second workgroup-memory bank.
    w_next = al.make_local((64,), al.bf16)
    k_next = al.make_local((64,), al.bf16)
    u_current = al.make_local((16,), al.bf16)
    u_next = al.make_local((16,), al.bf16)

    # Prologue: issue chunk 0 loads, then materialize the sole W/K LDS bank.
    for rep in al.range(64):
        linear = tid + rep * WORKGROUP
        k_half = linear // (BT * 64)
        rem = linear - k_half * (BT * 64)
        token_off = rem // 64
        local_k = rem - token_off * 64
        w_next[rep] = w[0, token_off, value_head_idx, k_half * 64 + local_k]
        k_next[rep] = k[0, token_off, key_head_idx, k_half * 64 + local_k]
    for rep in al.range(16):
        linear = tid + rep * WORKGROUP
        token_off = linear // BV
        local_v = linear - token_off * BV
        u_current[rep] = u[0, token_off, value_head_idx, value_base + local_v]
    if tid < BT:
        g_stage[tid] = g[0, tid, value_head_idx]
    for rep in al.range(64):
        linear = tid + rep * WORKGROUP
        k_half = linear // (BT * 64)
        rem = linear - k_half * (BT * 64)
        token_off = rem // 64
        local_k = rem - token_off * 64
        w_bf16[k_half, token_off, local_k] = w_next[rep]
        # block_dot's fixed C0 consumer reads [K, token].
        k_stage[k_half, local_k, token_off] = k_next[rep]
    al.syncthreads()
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # Triton-matched lookahead part 1: current W has already been staged,
        # so the next chunk's W global reads can overlap current pred/update.
        if chunk_idx + 1 < num_chunks:
            next_start = chunk_start + BT
            for rep in al.range(64):
                linear = tid + rep * WORKGROUP
                k_half = linear // (BT * 64)
                rem = linear - k_half * (BT * 64)
                ...
                local_k = rem - token_off * 64
                w_next[rep] = w[
                    0, next_start + token_off, value_head_idx, k_half * 64 + local_k
                ]

        # This is P0's corrected MFMA32 pred mapping. W now comes from the
        # prologue/tail LDS bank, but MFMA geometry and operand mapping remain
        # unchanged.
        for token_tile in al.range(2):
            ...
            for rep_out in al.range(8):
                ...
                corrected = al.convert(u_current[token_tile * 8 + rep_out], al.f32) - pred
                ...
                decay = al.exp(g_last - g_stage[token_base + token_off])
                if emit_audit:
        ...
        # Triton-matched lookahead part 2: issue next U/g/K global reads only
        # after current pred consumes its shared partials. They remain local
        # while current direct-K64 update reads the current K LDS bank.
        if chunk_idx + 1 < num_chunks:
            next_start = chunk_start + BT
            for rep in al.range(16):
                linear = tid + rep * WORKGROUP
                token_off = linear // BV
                local_v = linear - token_off * BV
                u_next[rep] = u[0, next_start + token_off, value_head_idx, value_base + local_v]
            if tid < BT:
                # Current correction has consumed g_stage, so this small
                # shared buffer can become the next-chunk g lookahead now.
                g_stage[tid] = g[0, next_start + tid, value_head_idx]
            for rep in al.range(64):
                linear = tid + rep * WORKGROUP
                k_half = linear // (BT * 64)
                rem = linear - k_half * (BT * 64)
                token_off = rem // 64
                local_k = rem - token_off * 64
                k_next[rep] = k[0, next_start + token_off, key_head_idx, k_half * 64 + local_k]

        # The staged C0 op keeps its K32 accumulation order. Preloaded-K is a
        # narrow source-to-lowering handoff: this op reads the current LDS bank
        # and cannot issue another global K load.
        ...
        # Current W/K LDS operands are dead. Tail-overwrite the one bank with
        # values loaded above; no second full LDS allocation or V-new reload.
        if chunk_idx + 1 < num_chunks:
            for rep in al.range(64):
                linear = tid + rep * WORKGROUP
                k_half = linear // (BT * 64)
...
def _maybe_dump_hsaco(launch: Callable[[], None]) -> None:
    ...
    target = _HSACO_DUMP_DIR / f"{_qwen_gdn_direct_k64_bv32_full_sequence_b2_lookahead_kernel.fn.__name__}.hsaco"
    if target.exists():
        launch()
        return
    ...
    def wrapped_compile(self, src, target_info, options=None):
        ...
        if not dumped and "full_sequence_b2_lookahead_kernel" in src.fn.fn.__name__:
    ...
    if not dumped:
        raise RuntimeError("B2 HSACO dump requested, but no matching kernel compiled")
...
def _run_kernel(
    ...
    def launch() -> None:
        _qwen_gdn_direct_k64_bv32_full_sequence_b2_lookahead_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
            k, w, u, g, initial_state, h, pred_f32, pred_bf16, v_new, v_decay,
            state_after, final_state, num_tokens, num_chunks, True, num_warps=2,
        )
...
def run_body(
    ...
    def launch() -> None:
        _qwen_gdn_direct_k64_bv32_full_sequence_b2_lookahead_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
            k, w, u, g, initial_state, h, pred_f32, pred_bf16, v_new, v_decay,
            state_after, final_state, num_tokens, num_chunks, False, num_warps=2,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "rocprof_outputs/qwen_direct_k64_bv32_full_sequence_b2_lookahead",
    )
    ...
    (args.out_dir / "b2_lookahead_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
    ...
    if not all(bool(result["pass"]) for result in results):
        raise SystemExit("B2 correctness gate failed; do not run resource or performance collection.")
