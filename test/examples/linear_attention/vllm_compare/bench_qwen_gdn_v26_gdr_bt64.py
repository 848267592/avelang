#!/usr/bin/env python3
"""Chunk_gdr-only benchmark for the v26 BT64/BV32 register-state experiment."""
...
def run_case(t: int, args: argparse.Namespace) -> dict[str, object]:
    ...
    # Compile paths before timing.  v23/v24 chunk_gdr baseline is the same v23 kernel.
    v23_fn()
    h26, vn26, fs26 = v26_fn(args.variant)
    torch.cuda.synchronize()

    h_abs = h_rel = vn_abs = vn_rel = fs_abs = fs_rel = float("nan")
    if args.check_ref and args.variant == MODE_FULL_V26C:
        ...
        h_abs, h_rel = max_err(h26, h_ref)
        vn_abs, vn_rel = max_err(vn26, vn_ref)
        fs_abs, fs_rel = max_err(fs26, fs_ref)

    v23_ms = time_fn(v23_fn, args.warmup, args.repeat)
    v26_ms = time_fn(lambda: v26_fn(args.variant), args.warmup, args.repeat)

    result = {
        "T": t,
        "variant": args.variant,
        "v23_v24_chunk_gdr_ms": v23_ms,
        "v26_chunk_gdr_ms": v26_ms,
        "speedup_v26_vs_v23": v23_ms / v26_ms,
        "h_abs": h_abs,
        "h_rel": h_rel,
        "vn_abs": vn_abs,
        "vn_rel": vn_rel,
    ...
    print(
        f"result,T={t},variant={args.variant},v23_v24_chunk_gdr_ms={v23_ms:.6f},"
        f"v26_chunk_gdr_ms={v26_ms:.6f},speedup_v26_vs_v23={v23_ms / v26_ms:.4f},"
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=VARIANTS, default=MODE_FULL_V26C)
    ...
    print(f"benchmark=chunk_gdr_only,v23_v24_chunk=16,v26_chunk=64,variant={args.variant}")

    rows = [run_case(t, args) for t in args.T]

    print("summary_table")
    print("T,v23_v24_chunk_gdr_ms,v26_chunk_gdr_ms,speedup,h_abs,h_rel,vn_abs,vn_rel,final_abs,final_rel")
    for r in rows:
        print(
            f"{r['T']},{r['v23_v24_chunk_gdr_ms']:.6f},{r['v26_chunk_gdr_ms']:.6f},"
            f"{r['speedup_v26_vs_v23']:.6f},{r['h_abs']:.6g},{r['h_rel']:.6g},"
            f"{r['vn_abs']:.6g},{r['vn_rel']:.6g},{r['final_abs']:.6g},{r['final_rel']:.6g}"
        )
