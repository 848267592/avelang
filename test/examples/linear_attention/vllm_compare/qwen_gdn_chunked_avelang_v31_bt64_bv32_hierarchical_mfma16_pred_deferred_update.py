"""Qwen GDN v31 MFMA16-pred deferred-update deferred-update hierarchical BT64/BV32 chunk_gdr experiment.

This file is intentionally isolated from the production v23/v24 baselines.

The kernel is a scheduling experiment for the v31 MFMA16-pred deferred-update path:

* BT=64, BV=32, workgroup=128 threads / 2 waves.
* Pred uses two K64 partial MFMA16 schedules for the active V16 tile.
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update(
    ...
    # These tensors describe exactly the active V16/token16 pred tile. The
    # two waves hold K[0:64] and K[64:128] partials respectively.
    state_bf16 = al.make_shared((2, 16, 64), al.bf16)
    w_bf16 = al.make_shared((2, 16, 64), al.bf16)
    pred_partial = al.make_shared((2, 16, 16), al.f32)
    # The V16 tile stays active, but its four token16 corrections must be
    # reduced before state writeback. This preserves the BT64 update order.
    v_decay_t_bf16 = al.make_shared((16, BT), al.bf16)
    k_tile_t = al.make_shared((16, BT), al.bf16)

    state_vec = al.view(
        state_bf16,
        al.i32,
        al.make_layout((2, 16, 8, 4), (16 * 8 * 4, 8 * 4, 4, 1)),
    )
    w_vec = al.view(
        w_bf16,
        al.i32,
    ...
    vdecay_vec = al.view(v_decay_t_bf16, al.i32, al.make_layout((16, 8, 4), (32, 4, 1)))
    ktile_vec = al.view(k_tile_t, al.i32, al.make_layout((16, 8, 4), (32, 4, 1)))
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # Complete one V16 block before beginning the other V16 block.
        for v_tile in al.range(2):
            for rep_state in al.range(16):
                linear_s = lane + rep_state * 64
                row_s = linear_s // 64
                col_s = linear_s - row_s * 64
                source_v_s = v_tile * 16 + row_s
                state_bf16[wave_id, row_s, col_s] = al.convert(state[source_v_s, wave_id * 64 + col_s], al.bf16)
            ...
            for token_tile in al.range(4):
                ...
                for rep_w in al.range(16):
                    linear_w = lane + rep_w * 64
                    row_w = linear_w // 64
                    col_w = linear_w - row_w * 64
                    token_idx_w = chunk_start + token_base + row_w
                    global_k_w = wave_id * 64 + col_w
                    w_bf16[wave_id, row_w, col_w] = al.convert(w[0, token_idx_w, value_head_idx, global_k_w], al.bf16)
                ...
                for r_pred in al.range(4):
                    pred_partial[wave_id, lane_group * 4 + r_pred, lane_col] = pred_acc[r_pred]
            ...
            # All token16 pred/v_decay work is now complete for this V16.
            # Reduce the BT64 contribution in a single MFMA16 accumulator per
            # V16xK16 tile, then apply the chunk decay exactly once.
            for k_tile in al.range(8):
                for rep_k in al.range(8):
                    linear_k = tid + rep_k * WORKGROUP
                    local_k = linear_k // BT
                    local_t = linear_k - local_k * BT
                    k_tile_t[local_k, local_t] = k[0, chunk_start + local_t, key_head_idx, k_tile * 16 + local_k]
                ...
                if wave_id == 0:
                    for token_sub in al.range(4):
                        ...
                        if lane_group == 0:
                            a_words_u16 = vdecay_vec[lane_col, pack_base]
                            b_words_u16 = ktile_vec[lane_col, pack_base]
                        ...
                        if lane_group == 2:
                            a_words_u16 = vdecay_vec[lane_col, pack_base + 1]
                            b_words_u16 = ktile_vec[lane_col, pack_base + 1]
                            a_frag_u16 = al.view(a_words_u16, al.Tensor((2, 4, 1), al.bf16))
                            b_frag_u16 = al.view(b_words_u16, al.Tensor((2, 4, 1), al.bf16))
                            update_acc16 = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_u16[0], b_frag_u16[0], update_acc16)
                        if lane_group == 3:
...
def _validate_inputs(
    ...
    if chunk_size != BT:
        raise ValueError("v31 MFMA16-pred deferred-update deferred-update hierarchical chunk_gdr only supports chunk_size=64.")
    ...
    if num_tokens % BT != 0:
        raise ValueError("v31 MFMA16-pred deferred-update deferred-update hierarchical chunk_gdr requires T divisible by 64.")
...
def qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_chunk_gdr_full_bf16_kernel_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update")
    launch()
...
def _smoke_main() -> None:
    ...
    h, final_state = qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update(
        k,
        w,
        u,
        gdr_decay,
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update(k, w, u, gdr_decay, gdr_g_last_exp, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update_ms={latency:.6f}")
...
__all__ = [
    ...
    "qwen_gdn_fused_chunk_gdr_full_avelang_v31_bt64_bv32_hierarchical_mfma16_pred_deferred_update",
