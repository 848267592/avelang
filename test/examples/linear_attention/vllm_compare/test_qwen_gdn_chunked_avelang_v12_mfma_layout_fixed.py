...
def _build_chunk_o_inputs(k, v, g, beta, initial_state, chunk_size: int):
    ...
    return g_cumsum, h, vn, final_state
...
def test_v12_chunk_o_mfma_matches_v9_chunk_o(t: int, with_initial_state: bool):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=3000 + t + int(with_initial_state))
    scale = 128 ** -0.5
    g_cumsum, h, vn, _ = _build_chunk_o_inputs(k, v, g, beta, initial_state, chunk)

    out_ref = qwen_gdn_chunk_o_avelang_v9_vllm_layout(
        q,
        k,
        vn,
        h,
    ...
    out_new = qwen_gdn_chunk_o_avelang_v12_mfma_layout(q, k, vn, h, g_cumsum, scale=scale, chunk_size=chunk)
    _assert_close("chunk_o_output", out_new, out_ref, atol=2.5e-1, rtol=2.5e-1)


@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v12_full_forward_matches_v11_update_mfma(t: int, with_initial_state: bool):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=4000 + t + int(with_initial_state))
    scale = 128 ** -0.5
    ...
    out_new, fs_new = qwen_gdn_chunked_avelang_v12_mfma_layout(
        q,
        k,
        v,
        g,
    ...
    _assert_close("final_state", fs_new, fs_ref, atol=1e-4, rtol=1e-4)
