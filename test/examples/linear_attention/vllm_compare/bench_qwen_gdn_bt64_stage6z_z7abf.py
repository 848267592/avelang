#!/usr/bin/env python3
"""Fresh-process paired isolated-body benchmark for Stage 6Z Z7AB-F."""
...
def _launch(arm: str, tensors: tuple[torch.Tensor, ...], output: torch.Tensor) -> Callable[[], None]:
    ...
    if arm == "z7abf":
        return lambda: qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z7abf_fusion_only_launch_into(
            *tensors, output
        )
...
def _parent(args: argparse.Namespace) -> None:
    ...
    paired_z7abf = [(value - base) * 1.0e3 for base, value in zip(medians["z5b"], medians["z7abf"])]
    summary = {
        arm: {
    ...
    payload = {
        ...
        "paired_differences_us": {
            "z7abf_minus_z5b": {
                "samples": paired_z7abf,
                "mean_us": statistics.fmean(paired_z7abf),
                "bootstrap95_us": _bootstrap_ci(paired_z7abf, 2026081900 + args.T),
            },
            "z7abf_over_native": [value / base for base, value in zip(medians["native"], medians["z7abf"])],
        },
        "raw": raw,
    }
