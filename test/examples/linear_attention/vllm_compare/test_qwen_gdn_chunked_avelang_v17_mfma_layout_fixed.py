...
def _build_gdr_inputs(k: torch.Tensor, v: torch.Tensor, g: torch.Tensor, beta: torch.Tensor, chunk_size: int):
...
def test_v17_gdr_decay_matches_torch(t: int):
    chunk = 16
    _, _, _, g, _, _ = _make_inputs(t, False, seed=17500 + t)
    ...
    num_chunks = t // chunk
    g_chunks = g_cumsum.reshape(1, num_chunks, chunk, 8)
    g_last = g_chunks[:, :, 15, :]
    decay_ref = torch.exp(g_last.unsqueeze(-1) - g_chunks.permute(0, 1, 3, 2))
    last_ref = torch.exp(g_last)

    _assert_close("gdr_decay", gdr_decay, decay_ref.contiguous(), atol=1e-6, rtol=1e-6)
    _assert_close("gdr_g_last_exp", gdr_g_last_exp, last_ref.contiguous(), atol=1e-6, rtol=1e-6)


@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v17_4wave_chunk_gdr_matches_v16(t: int, with_initial_state: bool):
    chunk = 16
    _, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=18000 + t + int(with_initial_state))
    g_cumsum, w, u = _build_gdr_inputs(k, v, g, beta, chunk)

    h_ref, vn_ref, fs_ref = qwen_gdn_chunk_gdr_avelang_v16_2wave_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    h_new, vn_new, fs_new = qwen_gdn_chunk_gdr_avelang_v17_4wave_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    gdr_decay, gdr_g_last_exp = qwen_gdn_gdr_decay_avelang_v17(g_cumsum, chunk_size=chunk)
    h_pre, vn_pre, fs_pre = qwen_gdn_chunk_gdr_avelang_v17_4wave_predecay_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
    ...
    _assert_close("h_predecay", h_pre, h_new)
    _assert_close("vn_predecay", vn_pre, vn_new)
    _assert_close("final_state_predecay", fs_pre, fs_new)


@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v17_full_forward_matches_v16(t: int, with_initial_state: bool):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(t, with_initial_state, seed=19000 + t + int(with_initial_state))
    scale = 128 ** -0.5

    out_ref, fs_ref = qwen_gdn_chunked_avelang_v16_mfma_layout(
        q,
        k,
        v,
        g,
    ...
    out_new, fs_new = qwen_gdn_chunked_avelang_v17_mfma_layout(
        q,
        k,
        v,
        g,
    ...
    out_pre, fs_pre = qwen_gdn_chunked_avelang_v17_predecay_mfma_layout(
        q,
        k,
        v,
        g,
    ...
    _assert_close("final_state", fs_new, fs_ref)
    _assert_close("output_predecay", out_pre, out_new)
    _assert_close("final_state_predecay", fs_pre, fs_new)
