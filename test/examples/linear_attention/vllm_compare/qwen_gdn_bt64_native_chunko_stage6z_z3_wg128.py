"""Opt-in Stage 6Z Z3 long-text WG128 chunk-o.

Z3 is the only long-text candidate after the fresh native selector capture
showed WG128/2-warp/2-stage at T=4096 and T=16384.  It keeps Z2's direct
fragment path and phase barriers, but maps one row-half to each of two waves.
Each wave visits the two value halves sequentially, so the [64, 64] tile and
the MFMA32 work remain unchanged while the workgroup is reduced to 128.
"""
...
@avelang.jit
def _qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_z3(
    ...
    # Two waves own row halves.  Each accumulator pair is consumed by that
    # wave for value halves 0 and 1 in sequence.
    inter_acc0 = al.full((16,), 0.0, al.f32)
    inter_acc1 = al.full((16,), 0.0, al.f32)

    # Phase A: one Q/H publication per K32 tile, then both value halves are
    # consumed by the same two waves before phase reuse.
    for k_stage in al.range(4):
        ...
        for value_half in al.range(2):
            for kt in al.range(2):
                ...
                if value_half == 0:
                    inter_acc0 = al.amdgpu.mfma_32x32x8_bf16_f32(h_frag[0], q_frag[0], inter_acc0)
                    inter_acc0 = al.amdgpu.mfma_32x32x8_bf16_f32(h_frag[1], q_frag[1], inter_acc0)
                else:
                    inter_acc1 = al.amdgpu.mfma_32x32x8_bf16_f32(h_frag[0], q_frag[0], inter_acc1)
                    inter_acc1 = al.amdgpu.mfma_32x32x8_bf16_f32(h_frag[1], q_frag[1], inter_acc1)
        al.syncthreads()

    # Phase B: both waves are score owners.  The two score halves retain the
    # Z2 publication boundary because phase is reused by the V stage.
    for source_half in al.range(2):
        ...
        for k_stage in al.range(4):
            ...
            # The score consumer reads only rows [0, 32) of this K plane.
            # Keeping the old 16-repetition Q/K loop issued dead K loads for
            # rows [32, 64); on the final chunk those addresses crossed K.
    ...
    # Phase C: one V publication, then each wave evaluates both value halves
    # sequentially with the already published score fragments.
    ...
    intra_acc0 = al.full((16,), 0.0, al.f32)
    intra_acc1 = al.full((16,), 0.0, al.f32)
    for source_half in al.range(2):
        for kt in al.range(2):
            word = kt * 2 + lane_group
            score_words = phase_vec[(row_half * 32 + lane_col) * 2 + source_half, word]
            v_words = phase_vec[128 + (lane_col) * 2 + source_half, word]
            score_frag = al.view(score_words, al.Tensor((2, 4, 1), al.bf16))
            v_frag = al.view(v_words, al.Tensor((2, 4, 1), al.bf16))
            intra_acc0 = al.amdgpu.mfma_32x32x8_bf16_f32(v_frag[0], score_frag[0], intra_acc0)
            intra_acc0 = al.amdgpu.mfma_32x32x8_bf16_f32(v_frag[1], score_frag[1], intra_acc0)
            v_words = phase_vec[128 + (32 + lane_col) * 2 + source_half, word]
            v_frag = al.view(v_words, al.Tensor((2, 4, 1), al.bf16))
            intra_acc1 = al.amdgpu.mfma_32x32x8_bf16_f32(v_frag[0], score_frag[0], intra_acc1)
            intra_acc1 = al.amdgpu.mfma_32x32x8_bf16_f32(v_frag[1], score_frag[1], intra_acc1)

    for r in al.range(16):
        out_col = ((r >> 2) << 3) + lane_group * 4 + (r & 3)
        token_offset = row_half * 32 + lane_col
        decay = al.exp(g[0, chunk_start + token_offset, value_head_idx])
        out[0, chunk_start + token_offset, value_head_idx, value_base + out_col] = al.convert(
            inter_acc0[r] * decay + intra_acc0[r], al.bf16
        )
        out[0, chunk_start + token_offset, value_head_idx, value_base + 32 + out_col] = al.convert(
            inter_acc1[r] * decay + intra_acc1[r], al.bf16
        )


def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z3_launch_into(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    *,
    ...
    """Caller-owned Z3 launch. This experimental candidate has no fallback."""
    ...
    if (
        ...
        raise ValueError("Stage 6Z Z3 output must be contiguous BF16 [1,T,8,128] on the input device.")
    if scale is None:
        scale = K_DIM ** -0.5
...
def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z3(
    q: torch.Tensor,
    k: torch.Tensor,
    v_new_bf16: torch.Tensor,
    h_bf16: torch.Tensor,
