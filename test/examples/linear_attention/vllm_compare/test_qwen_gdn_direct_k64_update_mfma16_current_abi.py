"""Correctness tests for the direct-K64 MFMA16 control update suffix."""

import pytest
import torch

from repro_qwen_gdn_direct_k64_update_mfma16_current_abi import (
    _make_inputs,
    qwen_gdn_direct_k64_update_mfma16_current_abi,
    qwen_gdn_direct_k64_update_reference,
)


@pytest.mark.parametrize("tokens", [64, 512])
def test_direct_k64_update_mfma16_current_abi(tokens: int) -> None:
    if not torch.cuda.is_available():
        pytest.skip("requires CUDA/HIP")
    k, v_new, g, initial = _make_inputs(tokens, 20260727 + tokens)
    actual_h, actual_final = qwen_gdn_direct_k64_update_mfma16_current_abi(k, v_new, g, initial)
