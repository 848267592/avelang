...
from qwen_gdn_chunked_avelang_v29_mfma32_fused_chunk_gdr_full_k_subtile_exp import (
    qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32,
)
...
def run_t(t: int, warmup: int, repeat: int) -> dict[str, float]:
    ...
    h, fs = qwen_gdn_fused_chunk_gdr_full_avelang_v29_mfma32(k, w, u, gdr_decay, gdr_g_last_exp, initial_state)
    sync()
    h_exp, fs_exp = qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32(
    ...
    h_exp_err = (h_exp - h_ref).abs()
    fs_exp_err = (fs_exp - fs_ref).abs()
    h_exp_vs_orig = (h_exp - h).abs()
    fs_exp_vs_orig = (fs_exp - fs).abs()

    # Isolate update correctness: w=0 removes pred feedback while preserving
    # the fused update dataflow.
    w_zero = torch.zeros_like(w)
    h_zero, fs_zero = qwen_gdn_fused_chunk_gdr_full_avelang_v29_mfma32(k, w_zero, u, gdr_decay, gdr_g_last_exp, initial_state)
    sync()
    h_zero_exp, fs_zero_exp = qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32(
    ...
    fs_zero_exp_err = (fs_zero_exp - fs_zero_ref).abs()

    f_v29_full = lambda: qwen_gdn_fused_chunk_gdr_full_avelang_v29_mfma32(
        k, w, u, gdr_decay, gdr_g_last_exp, initial_state
    )
    f_v29_k_subtile = lambda: qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32(
        k, w, u, gdr_decay, gdr_g_last_exp, initial_state
    )
    ...
    row = {
        ...
        "v29_fused_chunk_gdr_full_ms": time_fn(f_v29_full, warmup, repeat),
        "v29_fused_chunk_gdr_full_k_subtile_ms": time_fn(f_v29_k_subtile, warmup, repeat),
        ...
        "k_subtile_h_max_abs": h_exp_err.max().item(),
        "k_subtile_h_mean_abs": h_exp_err.mean().item(),
        "k_subtile_final_state_max_abs": fs_exp_err.max().item(),
        "k_subtile_final_state_mean_abs": fs_exp_err.mean().item(),
        "k_subtile_vs_original_h_max_abs": h_exp_vs_orig.max().item(),
        "k_subtile_vs_original_final_state_max_abs": fs_exp_vs_orig.max().item(),
        "w_zero_final_state_max_abs": fs_zero_err.max().item(),
        "w_zero_final_state_mean_abs": fs_zero_err.mean().item(),
        "k_subtile_w_zero_final_state_max_abs": fs_zero_exp_err.max().item(),
        "k_subtile_w_zero_final_state_mean_abs": fs_zero_exp_err.mean().item(),
    }
    print(row)
    return row
...
def main() -> None:
    ...
    for row in rows:
        print(
            ...
            f"fused_chunk_gdr_full={row['v29_fused_chunk_gdr_full_ms']:.6f} "
            f"k_subtile_exp={row['v29_fused_chunk_gdr_full_k_subtile_ms']:.6f} "
            f"h_max_abs={row['h_max_abs']:.6e} final_state_max_abs={row['final_state_max_abs']:.6e} "
            f"k_subtile_final_state_max_abs={row['k_subtile_final_state_max_abs']:.6e} "
