...
def test_v23_chunk_gdr_matches_v17_predecay(t: int, with_initial_state: bool):
    chunk = 16
    _, k, v, g, beta, initial_state = _make_inputs(
        t,
        with_initial_state,
        seed=23000 + t + int(with_initial_state) * 101,
    )
    ...
    h_new, vn_new, fs_new = qwen_gdn_chunk_gdr_avelang_v23_4wave_distributed_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
...
@pytest.mark.parametrize("t", [16, 32, 64, 512])
@pytest.mark.parametrize("with_initial_state", [False, True])
def test_v23_full_matches_v17_predecay(t: int, with_initial_state: bool):
    chunk = 16
    q, k, v, g, beta, initial_state = _make_inputs(
        t,
        with_initial_state,
        seed=24000 + t + int(with_initial_state) * 101,
    )
    scale = 128 ** -0.5
    ...
    out_new, fs_new = qwen_gdn_chunked_avelang_v23_gdr_distributed_layout(
        q,
        k,
        v,
        g,
