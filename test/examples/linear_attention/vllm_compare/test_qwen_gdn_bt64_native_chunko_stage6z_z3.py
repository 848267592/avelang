"""Correctness gates for the Stage 6Z Z3 WG128 long-text candidate."""
...
@pytest.mark.parametrize("t", (16384, 8192, 4096, 2048, 512, 64))
def test_stage6z_z3_is_byte_exact_to_z2_and_within_stage6w_contract(t: int) -> None:
    q, k, vn, h, g = _inputs(t, 2026080800 + t)
    z1 = qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z1(q, k, vn, h, g)
    z2 = qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z2(q, k, vn, h, g)
    z3 = qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z3(q, k, vn, h, g)
    frozen = qwen_gdn_chunk_o_bt64_bf16_vnew_bf16_out_stage6w(q, k, vn, h, g)
    torch.cuda.synchronize()
    assert bool(torch.isfinite(z3).all().item())
    assert torch.equal(z3, z2)
    assert torch.equal(z3, z1)
    assert float((z3.float() - frozen.float()).abs().max().item()) <= ATOL


def test_stage6z_z3_zero_vnew_and_caller_owned_output_reuse() -> None:
    q, k, vn, h, g = _inputs(64, 2026080901)
    vn.zero_()
