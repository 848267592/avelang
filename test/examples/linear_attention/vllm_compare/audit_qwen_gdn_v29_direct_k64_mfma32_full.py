#!/usr/bin/env python3
"""Recurrence-only ABI and correctness gate for full direct-K64/MFMA32.

This deliberately does not time a full graph. It gives the direct-K64/MFMA32
experiment the BF16 W/U and FP32 g/initial-state boundary used by the
current-vLLM recurrence, then compares its three recurrence outputs.
"""
...
from qwen_gdn_chunked_avelang_v29_mfma32_fused_chunk_gdr_full_direct_k64_mfma32_exp import (  # noqa: E402
...
def _run_case(t: int, seed: int, case: str) -> dict[str, object]:
    ...
    direct_result = qwen_gdn_fused_chunk_gdr_full_direct_k64_mfma32_avelang_v29(
        k, w, u, decay, g_last, initial_state
    )
    direct_ref = qwen_gdn_fused_chunk_gdr_full_direct_k64_mfma32_reference(
        k, w, u, decay, g_last, initial_state
    )
    ...
    h, v_new, final_state = direct_result
    h_ref, v_new_ref, final_ref = direct_ref
    ...
    return {
        ...
        "direct_output_dtypes": {
            "h": str(h.dtype), "v_new": str(v_new.dtype), "final_state": str(final_state.dtype),
        },
        "direct_vs_its_fp32_reference": own,
        "direct_vs_current_vllm_recurrence": native_error,
...
def main() -> None:
    ...
    result = {
        "experiment": "v29_direct_k64_mfma32_current_vllm_bf16_abi",
