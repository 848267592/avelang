"""Correctness gates for the single Z7AB A+B joint-superphase candidate."""
...
def test_z7ab_is_bf16_byte_exact_to_z5b(t: int) -> None:
    q, k, vn, h, g = _inputs(t, 2026081500 + t)
    expected = qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z5b_direct_q_cache(q, k, vn, h, g)
    actual = qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z7ab_joint_qhk_superphase(q, k, vn, h, g)
...
def test_z7ab_caller_owned_zero_v_nan_prefilled_output(t: int) -> None:
...
def test_z7ab_shape_contract_is_fixed_wg256() -> None:
    assert WORKGROUP == 256 == Z7AB_WORKGROUP_CONTRACT
