"""Opt-in Stage 6Z Z2 phase-aware BT64/BV64/BK32 chunk-o body.

Z2 preserves Z1's ownership, math, MFMA32 geometry, and CTA-wide producer
barriers.  Its only schedule change is to remove the lane-private ``frag_words``
LDS round trip: a lane consumes the i32 words it already loaded from ``phase``
directly.  Barriers now delimit shared-phase lifetimes, not each fragment word.

Hard shape contract: this Avelang kernel is always WG256 (four wave64 waves).
It is not an autotuned native-vLLM kernel and has no WG128 fallback.  WG128 in
Stage 6Z belongs only to the separate Z3 source or to native Triton selections
at long sequence lengths.  Any Z2/native comparison must pin native's selected
configuration before the direct body launch; never call the Triton autotuner
directly from a mixed benchmark.
"""
...
BK = 32
WORKGROUP = 256
Z2_WORKGROUP_CONTRACT = 256


@avelang.jit
def _qwen_gdn_chunk_o_bf16_vnew_bf16_out_kernel_bt64_stage6z_z2(
    ...
    # The same 16 KiB physical buffer is phase-separated exactly as in Z1.
    # Z2 removes only the extra lane-private LDS fragment buffer.
    phase = al.make_shared((256, 32), al.bf16)
    phase_vec = al.view(phase, al.Tensor((256, 4, 4), al.i32))

    inter_acc = al.full((16,), 0.0, al.f32)

    # Phase A: Q/H staging -> inter-state MFMA.  The first barrier publishes
    # cross-wave producer data; the second releases the reused phase rows.
    for k_stage in al.range(4):
        ...
        # All waves have consumed this shared K32 tile before its next write.
        al.syncthreads()

    # Phase B: Q/K staging and score creation.  The source-half serialization
    # barriers remain: score producer waves and later score/V consumers share
    # the phase buffer across waves.
    for source_half in al.range(2):
        ...
        for k_stage in al.range(4):
            ...
            # The consumer reads only the first 32 rows of this K plane.
            # Keep the reference path free of the dead final-chunk overread
            # that was exposed while debugging the WG128 candidate.
            ...
            # Protect phase reuse by the next K32 producer, not fragment reuse.
            al.syncthreads()
        ...
        # Preserve Z1's score half publication boundary unchanged.
        al.syncthreads()

    # Phase C: V-new transpose and score-times-V MFMA.  A single publication
    # barrier protects both score halves and the V transpose.  The following
    # fragments are lane-local register values and phase is no longer reused.
...
def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z2_launch_into(
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    *,
    ...
    """Caller-owned Z2 launch. This experimental candidate has no fallback."""
    if WORKGROUP != Z2_WORKGROUP_CONTRACT:
        raise RuntimeError("Stage 6Z Z2 shape contract violation: WG must remain 256")
    ...
    if (
        ...
        raise ValueError("Stage 6Z Z2 output must be contiguous BF16 [1,T,8,128] on the input device.")
    if scale is None:
        scale = K_DIM ** -0.5
...
def qwen_gdn_chunk_o_bt64_native_chunko_stage6z_z2(
    q: torch.Tensor,
    k: torch.Tensor,
    v_new_bf16: torch.Tensor,
    h_bf16: torch.Tensor,
