"""Qwen GDN v29 MFMA32 fused chunk_gdr K-subtile experiment.
...
* The update K staging uses a narrow K-subtile pattern instead of broad
  k_all_t[128,BT] materialization.
...
@avelang.jit
def _qwen_gdn_fused_chunk_gdr_full_k_subtile_bf16_kernel_v29_mfma32(
    ...
    ksub_vec = al.view(
        k_sub_t,
        al.i32,
        al.make_layout((128, 2, 4), (2 * 4, 4, 1)),
    )
        ...
        for v_half in al.range(2):
            for local_tile_u in al.range(4):
                ...
                for token_sub in al.range(4):
                    for rep_k_sub in al.range(16):
                        linear_k_sub = tid + rep_k_sub * WORKGROUP
                        k_row_sub = linear_k_sub // 16
                        token_local_sub = linear_k_sub - k_row_sub * 16
                        token_k_sub = token_sub * 16 + token_local_sub
                    ...
                    if lane_group == 0:
                        ...
                        b_words_u16 = ksub_vec[base_k_u + lane_col, 0]
                    ...
                    if lane_group == 2:
                        ...
                        b_words_u16 = ksub_vec[base_k_u + lane_col, 1]
                        a_frag_u16 = al.view(a_words_u16, al.Tensor((2, 4, 1), al.bf16))
                        b_frag_u16 = al.view(b_words_u16, al.Tensor((2, 4, 1), al.bf16))
                        update_acc16 = al.amdgpu.mfma_16x16x16_bf16_f32(a_frag_u16[0], b_frag_u16[0], update_acc16)
                    if lane_group == 3:
...
def qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    gdr_decay: torch.Tensor,
    ...
    *,
    ...
    def launch() -> None:
        _qwen_gdn_fused_chunk_gdr_full_k_subtile_bf16_kernel_v29_mfma32[
    ...
    _maybe_dump_hsaco(launch, "_qwen_gdn_fused_chunk_gdr_full_k_subtile_bf16_kernel_v29_mfma32")
    launch()
...
def _smoke_main() -> None:
    ...
    h, final_state = qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32(
        k,
        w,
        u,
        gdr_decay,
    ...
    latency = _benchmark_cuda(
        lambda: qwen_gdn_fused_chunk_gdr_full_k_subtile_avelang_v29_mfma32(k, w, u, gdr_decay, gdr_g_last_exp, initial_state),
        warmup=args.warmup,
        repeat=args.repeat,
    )
    print(f"T={args.T} v29_fused_chunk_gdr_full_k_subtile_ms={latency:.6f}")
