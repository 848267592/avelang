#!/usr/bin/env python3
"""R1 joint_v1: compiler-owned W/K BT64 stage tokens over one LDS bank.
...
@avelang.jit
def _qwen_gdn_persistent_recurrence_r1_joint_v1_kernel(
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # The plan-owned next W/K packet loads issue before current pred. They
        # remain opaque values until tail commit, not source local arrays.
...
def _maybe_dump_hsaco(launch: Callable[[], None]) -> None:
    ...
    target = _HSACO_DUMP_DIR / f"{_qwen_gdn_persistent_recurrence_r1_joint_v1_kernel.fn.__name__}.hsaco"
    if target.exists():
        launch()
        return
...
def _run_kernel(
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r1_joint_v1_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "rocprof_outputs/qwen_persistent_recurrence_r1_joint_v1",
    )
    ...
    (args.out_dir / "r1_joint_v1_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
