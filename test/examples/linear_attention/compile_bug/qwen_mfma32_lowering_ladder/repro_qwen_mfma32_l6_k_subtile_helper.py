#!/usr/bin/env python3
"""Qwen-shaped L6 K-subtile helper prototype.

This is an isolated compiler/backend evidence repro.  It deliberately keeps the
Qwen-ish MFMA32 pred/v_decay shape from the lowering ladder, then compares the
bad broad K staging path with the validated K-subtile source pattern.
"""
...
VARIANTS = {
    ...
    "L6_k_subtile_helper_full_update_like": 10,
}
    ...
    if variant < 9 or variant == 10:
        ...
        if variant < 8 or variant == 10:
            ...
            for token_tile in al.range(2):
                ...
                if (variant >= 4 and variant <= 6) or variant == 10:
