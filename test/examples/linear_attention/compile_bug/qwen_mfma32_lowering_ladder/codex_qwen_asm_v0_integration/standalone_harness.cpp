...
int main(int argc, char **argv) {
  if (argc != 8) {
    std::cerr << "usage: asm_v0_harness <hsaco> <symbol> <T> <input-dir> <output-dir> <warmup> <repeat>\n";
    return 64;
  }
  try {
    const std::string hsaco = argv[1], symbol = argv[2], input_dir = argv[4], output_dir = argv[5];
    int32_t t = std::stoi(argv[3]);
    ...
    Check(hipModuleGetFunction(&function, module, symbol.c_str()), "hipModuleGetFunction");
