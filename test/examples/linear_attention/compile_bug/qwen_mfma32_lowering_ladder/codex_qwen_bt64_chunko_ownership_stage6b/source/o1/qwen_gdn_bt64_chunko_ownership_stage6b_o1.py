"""Opt-in Stage 6B BT64 ``chunk_o`` ownership O1 experiment for gfx942.
...
CTA.  O1 assigns a CTA to ``[token=64, value=32]`` instead.  The four waves
continue to own token16 rows; they cooperatively build the ten causal score
tiles once, retain them in LDS, and reuse them across two V16 subtiles.  This
is the only O1 change from O0: a smaller V tile and unrolled live region.
...
@avelang.jit
def _qwen_gdn_chunk_o_bf16_kernel_bt64_ownership_o1(
    ...
    # each of two CTAs for the same V32 range and also executes upper blocks
...
def _require_chunko_o1_inputs(
    ...
    if chunk_size != BT:
        raise ValueError("Stage 6B chunk-o-O1 only supports chunk_size=64.")
    ...
    if num_tokens % BT:
        raise ValueError("Stage 6B chunk-o-O1 requires T divisible by 64.")
    if tuple(q.shape) != (1, num_tokens, H_K, K_DIM) or tuple(k.shape) != tuple(q.shape):
        raise ValueError("Stage 6B chunk-o-O1 requires q/k=[1,T,4,128].")
    if tuple(v_new.shape) != (1, num_tokens, H_V, V_DIM) or tuple(g.shape) != (1, num_tokens, H_V):
        raise ValueError("Stage 6B chunk-o-O1 requires v_new=[1,T,8,128] and g=[1,T,8].")
    if tuple(h_bf16.shape) != (1, num_chunks, H_V, V_DIM, K_DIM):
        raise ValueError("Stage 6B chunk-o-O1 requires h_bf16=[1,T/64,8,128,128].")
    return num_tokens, num_chunks


def qwen_gdn_chunk_o_bt64_ownership_o1(
    q: torch.Tensor,
    k: torch.Tensor,
    v_new: torch.Tensor,
    h_bf16: torch.Tensor,
    ...
    *,
    ...
    """FP32-staging O1 chunk-o with one CTA per ``[chunk, head, V32]``."""
    num_tokens, num_chunks = _require_chunko_o1_inputs(q, k, v_new, h_bf16, g, chunk_size)
    if scale is None:
        scale = K_DIM ** -0.5
    output = torch.empty_like(v_new)
    grid_size = num_chunks * H_V * V_TILES
    _qwen_gdn_chunk_o_bf16_kernel_bt64_ownership_o1[lambda: ((grid_size, 1, 1), (WORKGROUP, 1, 1))](
        q, k, v_new, h_bf16, g, output, float(scale), num_tokens, num_chunks
    )
    return output


def qwen_gdn_full_bt64_stage6b_chunko_o1_stages(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    output_fp32 = qwen_gdn_chunk_o_bt64_ownership_o1(q, k, v_new, h_bf16, g_cumsum, scale=scale)
    return {
        "g_cumsum": g_cumsum,
        "a": a,
        "a_solved": a_solved,
...
def qwen_gdn_full_bt64_stage6b_chunko_o1(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    stages = qwen_gdn_full_bt64_stage6b_chunko_o1_stages(
...
__all__ = [
    "V_TILE",
    "WORKGROUP",
    "_qwen_gdn_chunk_o_bf16_kernel_bt64_ownership_o1",
