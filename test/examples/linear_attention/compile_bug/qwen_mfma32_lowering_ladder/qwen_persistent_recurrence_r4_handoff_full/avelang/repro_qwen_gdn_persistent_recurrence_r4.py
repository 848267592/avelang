#!/usr/bin/env python3
"""R4 joint_v4 full recurrence using the shared R1 semantic kernel.
...
BT = r1.BT
LOWERING = "gfx942_bt64_bv32_joint_v4"


def _set_r4_lowering() -> None:
    ...
    *,
    ...
    def launch() -> None:
        joint_v4._qwen_gdn_persistent_recurrence_r4_joint_v4_kernel[
            ...
            emit_audit, True, False, num_warps=2,
        )
...
def run_correctness_length(t: int, *, seed: int) -> dict[str, Any]:
    ...
    comparisons = {
        "r4_vs_r2": b0._compare(actual, r2_actual, label="r4_vs_r2"),
        "r4_vs_r1": b0._compare(actual, r1_actual, label="r4_vs_r1"),
        "r4_vs_r3": b0._compare(actual, r3_actual, label="r4_vs_r3"),
        "r4_vs_b0": b0._compare(actual, b0_actual, label="r4_vs_b0"),
        "r4_vs_p2": b0._compare(actual, microscope, label="r4_vs_p2_host_microscope"),
        "r4_vs_device_contract": b0._compare(actual, reference, label="r4_vs_device_contract"),
    }
...
def main() -> None:
    ...
    (args.out_dir / "r4_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
