#!/usr/bin/env python3
"""R2 joint_v2: interleaved W-before-pred and K-before-update stage tokens.
...
@avelang.jit
def _qwen_gdn_persistent_recurrence_r2_joint_v2_kernel(
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # R2 issues next W before pred. K is deliberately deferred until the
        # pred/V-new phase has produced its transient operand, then becomes
        # available before the current K update. Both remain opaque packets
        # until the single-bank tail commit.
...
def _maybe_dump_hsaco(launch: Callable[[], None]) -> None:
    ...
    target = _HSACO_DUMP_DIR / f"{_qwen_gdn_persistent_recurrence_r2_joint_v2_kernel.fn.__name__}.hsaco"
    if target.exists():
        launch()
        return
    ...
    def wrapped_compile(self, src, target_info, options=None):
        ...
        if not dumped and "persistent_recurrence_r2_joint_v2_kernel" in src.fn.fn.__name__:
    ...
    if not dumped:
        raise RuntimeError("R2 joint_v2 HSACO dump requested, but no matching kernel compiled")
...
def _run_kernel(
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r2_joint_v2_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
            k, w, u, g, initial_state, h, pred_f32, pred_bf16, v_new, v_decay,
            state_after, final_state, num_tokens, num_chunks, True, True, num_warps=2,
        )
...
def run_body(
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r2_joint_v2_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
            k, w, u, g, initial_state, h, pred_f32, pred_bf16, v_new, v_decay,
            state_after, final_state, num_tokens, num_chunks, False, True, num_warps=2,
        )
...
def main() -> None:
    ...
    (args.out_dir / "r2_joint_v2_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
