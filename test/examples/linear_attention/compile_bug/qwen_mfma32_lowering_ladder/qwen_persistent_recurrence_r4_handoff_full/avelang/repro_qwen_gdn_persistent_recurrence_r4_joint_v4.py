#!/usr/bin/env python3
"""R4 joint_v4: full typed W/K packet-to-dot recurrence pipeline.

Unlike B2, this source never materializes w_next/k_next/u_next local arrays.
It exposes only opaque W/K stage tokens. The joint planner validates and tags
the full recurrence plan; the post-outline compiler pass emits the BF16x8
loads at the next-stage issue point and their same-bank LDS commits at the
...
@avelang.jit
def _qwen_gdn_persistent_recurrence_bv_consume_kernel(
    k_ptr: al.Pointer(al.bf16),
    w_ptr: al.Pointer(al.bf16),
    u_ptr: al.Pointer(al.bf16),
    g_ptr: al.Pointer(al.f32),
    ...
    emit_audit: al.constexpr,
    persistent_semantic: al.constexpr,
    bv_consume: al.constexpr,
):
    """R4-tail recurrence with a logical V16/V32/V64 output tile.

    The target instruction is fixed MFMA32.  ``bv_consume=16`` therefore
    uses one padded V32 physical consumer and publishes only the first sixteen
    rows; ``bv_consume=64`` serially consumes two V32 physical tiles before
    the existing tail W/K issue and commit.  The physical W/K bank and every
    pred/update operation are otherwise the validated R4 path.
    """

    if bv_consume != 16 and bv_consume != 32 and bv_consume != 64:
        raise ValueError("bv_consume must be one of 16, 32, or 64")
    ...
    g = al.make_tensor(g_ptr, al.f32,
                       al.make_layout((1, num_tokens, H_V),
                                      (num_tokens * H_V, H_V, 1)))
    initial_state = al.make_tensor(
    ...
    # BV64 keeps the native V32 MFMA geometry.  The four waves form two
    # independent (V32,K128) ownership pairs; each pair keeps its FP32 state
    # in the existing 16-element MFMA accumulator fragments.
    physical_bv = 32
    num_v32_subtiles = (bv_consume + physical_bv - 1) // physical_bv
    value_tiles_per_head = KDIM // bv_consume
    ...
    mfma_lane_group = lane >> 5
    program_id = al.block_id(0)
    value_head_idx = program_id // value_tiles_per_head
    value_base = (program_id - value_head_idx * value_tiles_per_head) * bv_consume
    key_head_idx = value_head_idx >> 1

    value_subtile = wave_id >> 1
    key_wave = wave_id & 1
    owned_physical_base = value_base + value_subtile * physical_bv
    h_lo = al.full((16,), 0.0, al.f32)
    h_hi = al.full((16,), 0.0, al.f32)
    for acc_i in al.range(16):
        local_k = ((acc_i >> 2) * 8) + mfma_lane_group * 4 + (acc_i & 3)
        global_v = owned_physical_base + lane_row
        if key_wave == 0:
            h_lo[acc_i] = initial_state[0, value_head_idx, global_v, local_k]
            h_hi[acc_i] = initial_state[0, value_head_idx, global_v, 32 + local_k]
        else:
    ...
    # Serial V32 consumers reuse the original two K-half LDS regions; BV64
    # must not allocate a second complete state/V-decay bank.
    state_bf16 = al.make_shared((2, physical_bv, 64), al.bf16)
    w_bf16 = al.make_shared((2, BT, 64), al.bf16)
    pred_partial = al.make_shared((2, 32, physical_bv), al.f32)
    vdecay_stage = al.make_shared((1, physical_bv, BT), al.bf16)
    k_stage = al.make_shared((2, BT, 64), al.bf16)
    state_vec = al.view(state_bf16, al.i32,
                        al.make_layout((2, physical_bv, 8, 4),
                                       (physical_bv * 8 * 4, 8 * 4, 4, 1)))
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        # Each physical V32 sub-tile completes both K64 pred halves, the
        # BF16 boundary, both update halves, and its FP32 feedback before the
        # next V32 sub-tile reuses the same temporary state/V-decay storage.
        for v_subtile in al.range(num_v32_subtiles):
            valid_rows = physical_bv
            physical_base = value_base + v_subtile * physical_bv
            g_last = g[0, chunk_start + BT - 1, value_head_idx]

            for acc_i in al.range(16):
                local_k = ((acc_i >> 2) * 8) + mfma_lane_group * 4 + (acc_i & 3)
                global_v = owned_physical_base + lane_row
                if value_subtile == v_subtile:
                    if key_wave == 0:
                        h[0, chunk_idx, value_head_idx, global_v, local_k] = al.convert(h_lo[acc_i], al.bf16)
                        h[0, chunk_idx, value_head_idx, global_v, 32 + local_k] = al.convert(h_hi[acc_i], al.bf16)
                        state_bf16[key_wave, lane_row, local_k] = al.convert(h_lo[acc_i], al.bf16)
                        state_bf16[key_wave, lane_row, 32 + local_k] = al.convert(h_hi[acc_i], al.bf16)
                    else:
                        h[0, chunk_idx, value_head_idx, global_v, 64 + local_k] = al.convert(h_lo[acc_i], al.bf16)
                        h[0, chunk_idx, value_head_idx, global_v, 96 + local_k] = al.convert(h_hi[acc_i], al.bf16)
                        state_bf16[key_wave, lane_row, local_k] = al.convert(h_lo[acc_i], al.bf16)
            ...
            for token_tile in al.range(2):
                ...
                if value_subtile == v_subtile:
                    for kpack in al.range(4):
                        k_vec = kpack * 2 + mfma_lane_group
                        w_words = w_vec[key_wave, token_tile, lane_row, k_vec]
                        state_words = state_vec[key_wave, lane_row, k_vec]
                        w_frag = al.view(w_words, al.Tensor((2, 4, 1), al.bf16))
                        state_frag = al.view(state_words, al.Tensor((2, 4, 1), al.bf16))
                        pred_acc = al.amdgpu.mfma_32x32x8_bf16_f32(state_frag[0], w_frag[0], pred_acc)
                        pred_acc = al.amdgpu.mfma_32x32x8_bf16_f32(state_frag[1], w_frag[1], pred_acc)
                    for acc_i in al.range(16):
                        out_col = ((acc_i >> 2) * 8) + mfma_lane_group * 4 + (acc_i & 3)
                        pred_partial[key_wave, lane_row, out_col] = pred_acc[acc_i]
                al.syncthreads()

                # Always clear the whole physical V32 staging tile.  For
                # BV16 the inactive half is zero padding, never a stale LDS
                # operand for the update dot.
                if value_subtile == v_subtile:
                    if key_wave == 0:
                        for rep_out in al.range(16):
                            linear = lane + rep_out * 64
                            token_off = linear // physical_bv
                            local_v = linear - token_off * physical_bv
                            token_idx = chunk_start + token_base + token_off
                            global_v = physical_base + local_v
                            pred = pred_partial[0, token_off, local_v] + pred_partial[1, token_off, local_v]
                            corrected = al.convert(u[0, token_idx, value_head_idx, global_v], al.f32) - pred
                            corrected_bf16 = al.convert(corrected, al.bf16)
                            corrected_from_bf16 = al.convert(corrected_bf16, al.f32)
            ...
            for k_half in al.range(2):
                update_pair = al.amdgpu.block_dot_bf16_f32_staged_vdecay_preloaded_k(
                    ...
                    value_head_idx, key_head_idx, physical_base, k_half, g_last,
                    h_lo, h_hi,
                )
                for acc_i in al.range(16):
                    if value_subtile == v_subtile:
        ...
        # R4-tail issue is deliberately outside all physical V32 consumers:
        # no next W/K load or same-bank commit is moved into a V sub-tile.
        al.syncthreads()
        next_w0 = al.amdgpu.qwen_bt64_pipeline_stage_load(w, tid, next_start, value_head_idx, 0)
        next_w1 = al.amdgpu.qwen_bt64_pipeline_stage_load(w, tid, next_start, value_head_idx, 1)
        next_k0 = al.amdgpu.qwen_bt64_pipeline_stage_load(k, tid, next_start, key_head_idx, 0)
...
@avelang.jit
def _qwen_gdn_persistent_recurrence_bv16_bv32_kernel(
    ...
    """One physical V32 consumer, logically masked to BV16 or BV32."""
        ...
        for acc_i in al.range(16):
            ...
            if wave_id == 0:
                if lane_row < bv_consume:
                    ...
                    state_bf16[0, lane_row, local_k] = al.convert(0.0, al.bf16)
                    state_bf16[0, lane_row, 32 + local_k] = al.convert(0.0, al.bf16)
            else:
                if lane_row < bv_consume:
                    h[0, chunk_idx, value_head_idx, global_v, 64 + local_k] = al.convert(h_lo[acc_i], al.bf16)
                    h[0, chunk_idx, value_head_idx, global_v, 96 + local_k] = al.convert(h_hi[acc_i], al.bf16)
                    ...
                    state_bf16[1, lane_row, local_k] = al.convert(0.0, al.bf16)
                    state_bf16[1, lane_row, 32 + local_k] = al.convert(0.0, al.bf16)
        al.syncthreads()
        ...
        for token_tile in al.range(2):
            ...
            for rep_out in al.range(8):
                ...
                if local_v < bv_consume:
                    ...
                    vdecay_stage[0, local_v, token_base + token_off] = al.convert(0.0, al.bf16)
            al.syncthreads()

        for k_half in al.range(2):
            update_pair = al.amdgpu.block_dot_bf16_f32_staged_vdecay_preloaded_k(
                vdecay_stage, k_stage, k, v_new, g, tid, chunk_start,
                value_head_idx, key_head_idx, value_base, k_half, g_last, h_lo, h_hi)
            for acc_i in al.range(16):
                if k_half == 0:
                    if wave_id == 0:
                        h_lo[acc_i] = update_pair[acc_i]
...
def _run_p2_host_microscope(
    ...
    """Replay the already-correct P1 body per chunk for byte-level B2 checks."""
    ...
    for chunk_idx in range(num_chunks):
        ...
        stop = start + BT
        case = p0.Case(
            f"b2_p2_T{num_tokens}_chunk{chunk_idx}",
...
def _maybe_dump_hsaco(launch: Callable[[], None]) -> None:
    ...
    target = _HSACO_DUMP_DIR / f"{_qwen_gdn_persistent_recurrence_r4_joint_v4_kernel.fn.__name__}.hsaco"
    if target.exists():
        launch()
        return
    ...
    def wrapped_compile(self, src, target_info, options=None):
        ...
        if not dumped and "persistent_recurrence_r4_joint_v4_kernel" in src.fn.fn.__name__:
    ...
    if not dumped:
        raise RuntimeError("R4 joint_v4 HSACO dump requested, but no matching kernel compiled")
...
def _run_kernel(
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r4_joint_v4_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
            k, w, u, g, initial_state, h, pred_f32, pred_bf16, v_new, v_decay,
            state_after, final_state, num_tokens, num_chunks, True, True, False, num_warps=2,
        )
...
def run_body(
    ...
    """Return the production-shape B2 body launch with audit stores compiled out.
    ...
    def launch() -> None:
        _qwen_gdn_persistent_recurrence_r4_joint_v4_kernel[lambda: ((GRID, 1, 1), (WORKGROUP, 1, 1))](
            k, w, u, g, initial_state, h, pred_f32, pred_bf16, v_new, v_decay,
            state_after, final_state, num_tokens, num_chunks, False, True, False, num_warps=2,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "rocprof_outputs/qwen_persistent_recurrence_r4_joint_v4",
    )
    parser.add_argument("--skip-p2", action="store_true", help="Only for fast compile/debug; not a B2 correctness gate.")
    ...
    (args.out_dir / "r4_joint_v4_correctness.json").write_text(json.dumps(results, indent=2) + "\n")
    if args.json:
    ...
    if not all(bool(result["pass"]) for result in results):
        raise SystemExit("R4 correctness gate failed; do not run resource or performance collection.")
