#!/usr/bin/env python3
"""Fresh-process diagnostic body benchmark for native full-recurrence B0.
...
def _find_repo_ladder() -> Path:
    """Find the original ladder when this handoff directory is copied alone."""
    for parent in (HERE, *HERE.parents):
        candidate = parent / "test/examples/linear_attention/compile_bug/qwen_mfma32_lowering_ladder"
        if candidate.is_dir():
            return candidate
    return HERE.parent / "compile_bug/qwen_mfma32_lowering_ladder"


LADDER = _find_repo_ladder()
...
class NativeBridge:
    """Existing Stage-6R bridge; B0 does not modify its code object or ABI."""
...
def _worker(t: int, *, seed: int, warmup: int, repeat: int, session: int) -> dict[str, Any]:
    ...
    calls = [_b0_call(k, w, u, g, initial_state), _direct_triton_call(k, w, u, g, initial_state), _bridge_call(k, w, u, g, initial_state)]
    ...
    if not all(value["h_vs_device_contract"]["finite"] and value["v_new_vs_device_contract"]["finite"] and value["final_state_vs_device_contract"]["finite"] for value in checks.values()):
        raise RuntimeError("non-finite output in B0 diagnostic comparison")
    ...
    base_order = [0, 1, 2, 2, 1, 0]
    order = base_order[session % 3 :] + base_order[: session % 3]
