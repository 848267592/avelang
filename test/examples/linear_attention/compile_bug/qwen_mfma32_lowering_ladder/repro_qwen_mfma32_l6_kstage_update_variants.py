#!/usr/bin/env python3
"""Qwen-shaped L6 K-staging plus real update-MFMA variants.

This is an isolated compiler/backend evidence repro.  It deliberately keeps the
Qwen-ish MFMA32 pred/v_decay shape from the lowering ladder, then varies how K
is staged before the MFMA16 update path.
"""
...
VARIANTS = {
    "L6_baseline_current_update": 0,
    "L6_khalf_stage64_one_update": 1,
    "L6_khalf_stage64_one_ktile_update": 2,
    "L6_khalf_stage64_full_update_like": 3,
    "L6_subtile16_stage_one_update": 4,
    "L6_subtile16_stage_one_ktile_update": 5,
    "L6_subtile16_stage_full_update_like": 6,
    "L6_no_shared_k_direct_global_update_probe": 7,
    "L6_update_mfma_no_pred_dependency": 8,
    "L6_update_mfma_minimal_frag": 9,
    "L6_fixed_kfrag_producer_consumer_rewrite": 10,
}
        ...
        if variant < 8 or variant == 10:
            ...
            for token_tile in al.range(2):
                ...
                if variant == 10:
                    ...
                    for rep_k_fixed in al.range(64):
                        linear_k_fixed = tid + rep_k_fixed * WORKGROUP
                        kk_fixed = linear_k_fixed // BT
                        tok_k_fixed = linear_k_fixed - kk_fixed * BT
                        k_all_t_fixed[kk_fixed, tok_k_fixed] = k[0, tok_k_fixed, key_head_idx, kk_fixed]
                    al.syncthreads()

                    for tile_fixed in al.range(8):
                        acc16_fixed = al.full((4,), 0.0, al.f32)
                        pack_base_fixed = token_tile * 4
                        k_col_fixed = tile_fixed * 16 + lane_col
                        if lane_group == 0:
                            a_words_fixed = vdecay_vec[lane_col, pack_base_fixed]
                            a_frag_fixed = al.view(a_words_fixed, al.Tensor((2, 4, 1), al.bf16))
                            b_frag_fixed = al.amdgpu.qwen_update_kfrag_load_bf16x4(
                                k_all_t_fixed,
                                k,
                                tid,
                                key_head_idx,
                            ...
                            acc16_fixed = al.amdgpu.mfma_16x16x16_bf16_f32(
                                a_frag_fixed[0], b_frag_fixed, acc16_fixed
                            )
                        if lane_group == 1:
                            a_words_fixed = vdecay_vec[lane_col, pack_base_fixed]
                            a_frag_fixed = al.view(a_words_fixed, al.Tensor((2, 4, 1), al.bf16))
                            ...
                            acc16_fixed = al.amdgpu.mfma_16x16x16_bf16_f32(
                                a_frag_fixed[1], b_frag_fixed, acc16_fixed
                            )
                        if lane_group == 2:
                            a_words_fixed = vdecay_vec[lane_col, pack_base_fixed + 1]
                            a_frag_fixed = al.view(a_words_fixed, al.Tensor((2, 4, 1), al.bf16))
                            b_frag_fixed = al.amdgpu.qwen_update_kfrag_load_bf16x4(
                                k_all_t_fixed,
                                k,
...
def check_rewrite_equivalence(seed: int) -> dict[str, object]:
    tensors = make_inputs(seed)
    launch_variant("L6_baseline_current_update", tensors)
    torch.cuda.synchronize()
    baseline = tensors[-1].clone()

    launch_variant("L6_fixed_kfrag_producer_consumer_rewrite", tensors)
    torch.cuda.synchronize()
    rewritten = tensors[-1].clone()
    diff = (baseline - rewritten).abs()
    return {
        "baseline_finite": bool(torch.isfinite(baseline).all().item()),
        "rewrite_finite": bool(torch.isfinite(rewritten).all().item()),
...
def main() -> None:
    ...
    parser.add_argument("--check-rewrite-equivalence", action="store_true")
    args = parser.parse_args()
    if args.check_rewrite_equivalence:
        print(json.dumps(check_rewrite_equivalence(args.seed), indent=2, sort_keys=True))
        return
