"""Clean Qwen GDN v17 MFMA prototype for the Qwen3Next TP4 per-rank target.
...
v17 keeps the v14 standalone MFMA w_u path and v14 chunk_o MFMA path, and
adds a 4-wave cooperative chunk_gdr kernel.  Each 256-thread block owns the
same [BV,128] state tile as v16, but the four waves split K into 32-column
quarters for pred/update work.
"""
...
@avelang.jit
def _qwen_gdn_w_bf16_kernel_v14_mfma(
    ...
    a = al.make_tensor(
        a_ptr,
        al.f32,
        al.make_layout((1, num_tokens, 8, 16), (num_tokens * 8 * 16, 8 * 16, 16, 1)),
    )
    w = al.make_tensor(
        w_ptr,
        al.f32,
    ...
    a_bf16 = al.make_shared((16, 16), al.bf16)
    b_bf16 = al.make_shared((16, 16), al.bf16)
    a_vec = al.view(a_bf16, al.i32, al.make_layout((16, 2, 4), (8, 4, 1)))
    b_vec = al.view(b_bf16, al.i32, al.make_layout((16, 2, 4), (8, 4, 1)))
...
@avelang.jit
def _qwen_gdn_u_bf16_kernel_v14_mfma(
...
@avelang.jit
def _qwen_gdn_chunk_gdr_bf16_kernel_v16_2wave_mfma(
    k_ptr: al.Pointer(al.bf16),
    w_ptr: al.Pointer(al.f32),
    u_ptr: al.Pointer(al.f32),
    g_ptr: al.Pointer(al.f32),
    ...
    v_decay_t = al.make_shared((BV, BT), al.bf16)
    k_all_t = al.make_shared((128, BT), al.bf16)
    pred_partial = al.make_shared((2, BT, BV), al.f32)
    ...
    for chunk_idx in al.range(num_chunks):
        ...
        for rep_h in al.range(16):
            idx_h = lane + rep_h * 64
            vv_h = idx_h // BK
            kk_local_h = idx_h - vv_h * BK
            kk_h = kk_local_h + wave_id * 64
            global_v_h = value_base + vv_h
            h[0, chunk_idx, value_head_idx, global_v_h, kk_h] = state[vv_h, kk_h]

        for rep_all in al.range(16):
            idx_all = lane + rep_all * 64
            row_all = idx_all // BK
            kk_local_all = idx_all - row_all * BK
            col_all = kk_local_all + wave_id * 64
            token_idx = chunk_start + row_all
            state_bf16[row_all, col_all] = al.convert(state[row_all, col_all], al.bf16)
            w_bf16[row_all, col_all] = al.convert(w[0, token_idx, value_head_idx, col_all], al.bf16)
            k_all_t[col_all, row_all] = k[0, token_idx, key_head_idx, col_all]

        al.syncthreads()

        pred_acc = al.full((4,), 0.0, al.f32)
        for batch64 in al.range(2):
            k_vec64 = lane_group + batch64 * 4 + wave_id * 8
            a_words = w_vec[lane_col, k_vec64]
            b_words = state_vec[lane_col, k_vec64]
            a_frag = al.view(a_words, al.Tensor((2, 4, 1), al.bf16))
            b_frag = al.view(b_words, al.Tensor((2, 4, 1), al.bf16))
        ...
        if wave_id == 0:
            for r in al.range(4):
                ...
                token_idx = chunk_start + token_offset
                global_v = value_base + value_offset
                pred_value = pred_partial[0, token_offset, value_offset] + pred_partial[1, token_offset, value_offset]
        ...
        for local_tile in al.range(4):
            global_tile = wave_id * 4 + local_tile
    ...
    for rep_final in al.range(16):
        ...
        vv_final = idx_final // BK
        kk_local_final = idx_final - vv_final * BK
        kk_final = kk_local_final + wave_id * 64
        global_v_final = value_base + vv_final
        final_state[0, value_head_idx, global_v_final, kk_final] = state[vv_final, kk_final]


@avelang.jit
def _qwen_gdn_gdr_decay_kernel_v17(
    g_ptr: al.Pointer(al.f32),
    ...
    chunk_start = chunk_idx * 16
    g_last = g[0, chunk_start + 15, value_head_idx]

    if lane == 0:
        gdr_g_last_exp[0, chunk_idx, value_head_idx] = al.exp(g_last)
    if lane < 16:
        g_value = g[0, chunk_start + lane, value_head_idx]
        gdr_decay[0, chunk_idx, value_head_idx, lane] = al.exp(g_last - g_value)


@avelang.jit
def _qwen_gdn_chunk_gdr_bf16_kernel_v17_4wave_mfma(
    k_ptr: al.Pointer(al.bf16),
    w_ptr: al.Pointer(al.f32),
    u_ptr: al.Pointer(al.f32),
    g_ptr: al.Pointer(al.f32),
...
def _is_v14_target_shape(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, g: torch.Tensor, beta: torch.Tensor | None = None) -> bool:
...
def _require_v14_target_shape(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, g: torch.Tensor, beta: torch.Tensor | None = None) -> None:
    if not _is_v14_target_shape(q, k, v, g, beta):
        raise ValueError(
            "v14 MFMA only supports Qwen3Next TP4 per-rank shape "
            "B=1,Hk=4,Hv=8,K=128,V=128,layout=[B,T,H,D]."
        )
    if q.dtype != torch.bfloat16 or k.dtype != torch.bfloat16 or v.dtype != torch.bfloat16:
        raise ValueError("v14 MFMA requires q/k/v dtype torch.bfloat16.")
    if g.dtype != torch.float32:
        raise ValueError("v14 MFMA requires g dtype torch.float32.")
    if beta is not None and beta.dtype != torch.float32:
        raise ValueError("v14 MFMA requires beta dtype torch.float32.")


def _validate_initial_state_v14(initial_state: torch.Tensor | None, *, device: torch.device) -> tuple[torch.Tensor | None, bool]:
...
def qwen_gdn_w_u_avelang_v14_mfma_layout(
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    beta: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v14 w_u MFMA only supports chunk_size=16.")
    if k.dtype != torch.bfloat16 or v.dtype != torch.bfloat16:
        raise ValueError("v14 w_u MFMA requires k/v dtype torch.bfloat16.")
    if g.dtype != torch.float32 or beta.dtype != torch.float32 or a_solved.dtype != torch.float32:
        raise ValueError("v14 w_u MFMA requires g/beta/a_solved dtype torch.float32.")
    if not k.is_cuda or not v.is_cuda or not g.is_cuda or not beta.is_cuda or not a_solved.is_cuda:
        raise ValueError("v14 w_u MFMA requires CUDA/HIP tensors.")
    if not k.is_contiguous() or not v.is_contiguous() or not g.is_contiguous() or not beta.is_contiguous() or not a_solved.is_contiguous():
        raise ValueError("v14 w_u MFMA requires contiguous tensors.")
    ...
    if tuple(a_solved.shape) != (1, num_tokens, 8, 16):
        raise ValueError("a_solved must have shape [1,T,8,16].")
    if num_tokens % BT != 0:
        raise ValueError("v14 w_u MFMA requires num_tokens divisible by 16.")
    ...
    _qwen_gdn_w_bf16_kernel_v14_mfma[lambda: ((grid_size, 1, 1), (64, 1, 1))](
        k,
        g,
        beta,
        a_solved,
    ...
    _qwen_gdn_u_bf16_kernel_v14_mfma[lambda: ((grid_size, 1, 1), (64, 1, 1))](
        v,
        beta,
        a_solved,
        u,
...
def qwen_gdn_chunk_gdr_avelang_v14_mfma_layout(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v14 chunk_gdr MFMA only supports chunk_size=16.")
    ...
    if (batch_size, num_k_heads, num_v_heads, head_dim_k, head_dim_v) != (1, 4, 8, 128, 128):
        raise ValueError("v14 chunk_gdr MFMA only supports B=1,Hk=4,Hv=8,K=128,V=128.")
    if num_tokens % BT != 0:
        raise ValueError("v14 chunk_gdr MFMA requires num_tokens divisible by 16.")
...
def qwen_gdn_chunk_gdr_avelang_v16_2wave_mfma_layout(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v16 2-wave chunk_gdr MFMA only supports chunk_size=16.")
    batch_size, num_tokens, num_k_heads, num_v_heads, head_dim_k, head_dim_v = _validate_bf16_chunk_gdr_stage(k, w, u, g, chunk_size)
    if (batch_size, num_k_heads, num_v_heads, head_dim_k, head_dim_v) != (1, 4, 8, 128, 128):
        raise ValueError("v16 2-wave chunk_gdr MFMA only supports B=1,Hk=4,Hv=8,K=128,V=128.")
    if num_tokens % BT != 0:
        raise ValueError("v16 2-wave chunk_gdr MFMA requires num_tokens divisible by 16.")
    initial_state_arg, has_initial_state = _validate_initial_state_v14(initial_state, device=k.device)
    ...
    _qwen_gdn_chunk_gdr_bf16_kernel_v16_2wave_mfma[lambda: ((64, 1, 1), (128, 1, 1))](
        k,
        w,
        u,
        g,
...
def qwen_gdn_gdr_decay_avelang_v17(
    g: torch.Tensor,
    *,
    chunk_size: int = BT,
) -> tuple[torch.Tensor, torch.Tensor]:
    if chunk_size != BT:
        raise ValueError("v17 gdr_decay only supports chunk_size=16.")
    _require_fp32_cuda_contiguous("g", g)
    if tuple(g.shape) != (1, g.shape[1], 8):
        raise ValueError("g must have shape [1,T,8].")
    num_tokens = g.shape[1]
    if num_tokens % BT != 0:
        raise ValueError("v17 gdr_decay requires num_tokens divisible by 16.")

    num_chunks = _num_chunks(num_tokens, chunk_size)
    gdr_decay = torch.empty((1, num_chunks, 8, 16), dtype=torch.float32, device=g.device)
    gdr_g_last_exp = torch.empty((1, num_chunks, 8), dtype=torch.float32, device=g.device)
    grid_size = num_chunks * 8
    _qwen_gdn_gdr_decay_kernel_v17[lambda: ((grid_size, 1, 1), (64, 1, 1))](
        g,
        gdr_decay,
        gdr_g_last_exp,
        num_tokens,
    ...
    return gdr_decay, gdr_g_last_exp


def qwen_gdn_chunk_gdr_avelang_v17_4wave_mfma_layout(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v17 4-wave chunk_gdr MFMA only supports chunk_size=16.")
    batch_size, num_tokens, num_k_heads, num_v_heads, head_dim_k, head_dim_v = _validate_bf16_chunk_gdr_stage(k, w, u, g, chunk_size)
    if (batch_size, num_k_heads, num_v_heads, head_dim_k, head_dim_v) != (1, 4, 8, 128, 128):
        raise ValueError("v17 4-wave chunk_gdr MFMA only supports B=1,Hk=4,Hv=8,K=128,V=128.")
    if num_tokens % BT != 0:
        raise ValueError("v17 4-wave chunk_gdr MFMA requires num_tokens divisible by 16.")
    initial_state_arg, has_initial_state = _validate_initial_state_v14(initial_state, device=k.device)
    ...
    _qwen_gdn_chunk_gdr_bf16_kernel_v17_4wave_mfma[lambda: ((64, 1, 1), (256, 1, 1))](
        k,
        w,
        u,
        g,
...
def qwen_gdn_chunk_gdr_avelang_v17_4wave_predecay_mfma_layout(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v17 4-wave predecay chunk_gdr MFMA only supports chunk_size=16.")
    batch_size, num_tokens, num_k_heads, num_v_heads, head_dim_k, head_dim_v = _validate_bf16_chunk_gdr_stage(k, w, u, g, chunk_size)
    if (batch_size, num_k_heads, num_v_heads, head_dim_k, head_dim_v) != (1, 4, 8, 128, 128):
        raise ValueError("v17 4-wave predecay chunk_gdr MFMA only supports B=1,Hk=4,Hv=8,K=128,V=128.")
    if num_tokens % BT != 0:
        raise ValueError("v17 4-wave predecay chunk_gdr MFMA requires num_tokens divisible by 16.")
    ...
    if gdr_decay.device != k.device or gdr_g_last_exp.device != k.device:
...
def qwen_gdn_chunk_o_avelang_v14_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    vn: torch.Tensor,
    h: torch.Tensor,
    ...
    *,
    ...
    if scale is None:
        scale = 128 ** -0.5
    if chunk_size != BT:
        raise ValueError("v14 chunk_o MFMA only supports chunk_size=16.")
    if q.dtype != torch.bfloat16 or k.dtype != torch.bfloat16:
        raise ValueError("v14 chunk_o MFMA requires q/k dtype torch.bfloat16.")
    if vn.dtype != torch.float32 or h.dtype != torch.float32 or g.dtype != torch.float32:
        raise ValueError("v14 chunk_o MFMA requires vn/h/g dtype torch.float32.")
    if tuple(q.shape) != (1, q.shape[1], 4, 128):
    ...
    if tuple(g.shape) != (1, num_tokens, 8):
        raise ValueError("g must have shape [1,T,8].")
    if num_tokens % BT != 0:
        raise ValueError("v14 chunk_o MFMA requires num_tokens divisible by 16.")
    num_chunks = _num_chunks(num_tokens, chunk_size)
...
def qwen_gdn_chunked_avelang_v14_mfma_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v14 full MFMA only supports chunk_size=16.")
    ...
    if q.shape[1] % BT != 0:
        raise ValueError("v14 full MFMA requires num_tokens divisible by 16.")
    ...
    h, vn, final_state = qwen_gdn_chunk_gdr_avelang_v14_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
...
def qwen_gdn_chunked_avelang_v14_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _, output, _, _, final_state = qwen_gdn_chunked_avelang_v14_mfma_layout_full(
        q,
        k,
        v,
        g,
...
def qwen_gdn_chunked_avelang_v16_mfma_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _validate_chunk_size(chunk_size)
    if chunk_size != BT:
        raise ValueError("v16 full MFMA only supports chunk_size=16.")
    _validate_bf16_qkvgb(q, k, v, g, beta)
    _require_v14_target_shape(q, k, v, g, beta)
    _validate_initial_state_v14(initial_state, device=q.device)
    if q.shape[1] % BT != 0:
        raise ValueError("v16 full MFMA requires num_tokens divisible by 16.")

    g_cumsum = qwen_gdn_chunk_cumsum_avelang_v6_standalone(g, chunk_size=chunk_size)
    a = qwen_gdn_kkt_avelang_v6_standalone(k, g_cumsum, beta, chunk_size=chunk_size, prefer_optimized=True)
    a_solved = qwen_gdn_solve_avelang_v6_standalone(a, chunk_size=chunk_size)
    w, u = qwen_gdn_w_u_avelang_v14_mfma_layout(k, v, g_cumsum, beta, a_solved, chunk_size=chunk_size)
    h, vn, final_state = qwen_gdn_chunk_gdr_avelang_v16_2wave_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
...
def qwen_gdn_chunked_avelang_v16_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if beta is None:
        beta = torch.ones_like(g, dtype=torch.float32, device=g.device).contiguous()
    _, output, _, _, final_state = qwen_gdn_chunked_avelang_v16_mfma_layout_full(
        q,
        k,
        v,
        g,
...
def qwen_gdn_chunked_avelang_v17_mfma_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _validate_chunk_size(chunk_size)
    if chunk_size != BT:
        raise ValueError("v17 full MFMA only supports chunk_size=16.")
    _validate_bf16_qkvgb(q, k, v, g, beta)
    _require_v14_target_shape(q, k, v, g, beta)
    _validate_initial_state_v14(initial_state, device=q.device)
    if q.shape[1] % BT != 0:
        raise ValueError("v17 full MFMA requires num_tokens divisible by 16.")

    g_cumsum = qwen_gdn_chunk_cumsum_avelang_v6_standalone(g, chunk_size=chunk_size)
    a = qwen_gdn_kkt_avelang_v6_standalone(k, g_cumsum, beta, chunk_size=chunk_size, prefer_optimized=True)
    a_solved = qwen_gdn_solve_avelang_v6_standalone(a, chunk_size=chunk_size)
    w, u = qwen_gdn_w_u_avelang_v14_mfma_layout(k, v, g_cumsum, beta, a_solved, chunk_size=chunk_size)
    h, vn, final_state = qwen_gdn_chunk_gdr_avelang_v17_4wave_mfma_layout(
        k,
        w,
        u,
        g_cumsum,
...
def qwen_gdn_chunked_avelang_v17_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if beta is None:
        beta = torch.ones_like(g, dtype=torch.float32, device=g.device).contiguous()
    _, output, _, _, final_state = qwen_gdn_chunked_avelang_v17_mfma_layout_full(
        q,
        k,
        v,
        g,
...
def qwen_gdn_chunked_avelang_v17_predecay_mfma_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _validate_chunk_size(chunk_size)
    if chunk_size != BT:
        raise ValueError("v17 predecay full MFMA only supports chunk_size=16.")
    _validate_bf16_qkvgb(q, k, v, g, beta)
    _require_v14_target_shape(q, k, v, g, beta)
    _validate_initial_state_v14(initial_state, device=q.device)
    if q.shape[1] % BT != 0:
        raise ValueError("v17 predecay full MFMA requires num_tokens divisible by 16.")
...
def qwen_gdn_chunked_avelang_v17_predecay_mfma_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if beta is None:
        beta = torch.ones_like(g, dtype=torch.float32, device=g.device).contiguous()
    _, output, _, _, final_state, _, _ = qwen_gdn_chunked_avelang_v17_predecay_mfma_layout_full(
        q,
        k,
        v,
        g,
...
__all__ = [
    ...
    "qwen_gdn_chunked_avelang_v14_mfma_layout_full",
    "qwen_gdn_chunked_avelang_v14_mfma_layout",
    "qwen_gdn_chunked_avelang_v16_mfma_layout_full",
    "qwen_gdn_chunked_avelang_v16_mfma_layout",
    "qwen_gdn_chunked_avelang_v17_mfma_layout_full",
    "qwen_gdn_chunked_avelang_v17_mfma_layout",
    "qwen_gdn_chunked_avelang_v17_predecay_mfma_layout_full",
