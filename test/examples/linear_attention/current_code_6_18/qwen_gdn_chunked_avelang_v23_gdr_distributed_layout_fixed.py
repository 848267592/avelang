"""Clean Qwen GDN v23 distributed-vn chunk_gdr production prototype.

v23 keeps the v17 predecay BT16 full path and replaces only chunk_gdr with
the v22-proven distributed vn/v_decay implementation.  It intentionally
does not expose profiling-only ablation modes.
"""
...
@avelang.jit
def _qwen_gdn_chunk_gdr_bf16_kernel_v23_4wave_distributed_vn_mfma(
...
def qwen_gdn_chunk_gdr_avelang_v23_4wave_distributed_mfma_layout(
    k: torch.Tensor,
    w: torch.Tensor,
    u: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v23 distributed chunk_gdr only supports chunk_size=16.")
    ...
    if (batch_size, num_k_heads, num_v_heads, head_dim_k, head_dim_v) != (1, 4, 8, 128, 128):
        raise ValueError("v23 distributed chunk_gdr only supports B=1,Hk=4,Hv=8,K=128,V=128.")
    if num_tokens % BT != 0:
        raise ValueError("v23 distributed chunk_gdr requires num_tokens divisible by 16.")
    num_chunks = _num_chunks(num_tokens, chunk_size)
    _validate_gdr_decay_v23(gdr_decay, gdr_g_last_exp, num_chunks=num_chunks, device=k.device)
    ...
    _qwen_gdn_chunk_gdr_bf16_kernel_v23_4wave_distributed_vn_mfma[lambda: ((64, 1, 1), (256, 1, 1))](
        k,
        w,
        u,
        gdr_decay,
...
def qwen_gdn_chunked_avelang_v23_gdr_distributed_layout_full(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    if chunk_size != BT:
        raise ValueError("v23 distributed full path only supports chunk_size=16.")
    ...
    if q.shape[1] % BT != 0:
        raise ValueError("v23 distributed full path requires num_tokens divisible by 16.")
...
def qwen_gdn_chunked_avelang_v23_gdr_distributed_layout(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor,
    ...
    *,
    ...
    _, output, _, _, final_state, _, _ = qwen_gdn_chunked_avelang_v23_gdr_distributed_layout_full(
        q,
        k,
        v,
        g,
...
__all__ = [
    ...
    "qwen_gdn_chunked_avelang_v23_gdr_distributed_layout_full",
