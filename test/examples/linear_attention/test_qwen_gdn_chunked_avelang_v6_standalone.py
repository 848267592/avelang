"""Qwen GDN v6 standalone：端到端正确性测试。

本文件验证 standalone v6 在当前支持范围内同时对齐 PyTorch reference 和现有 v6：
...
from qwen_gdn_chunked_avelang_v6 import qwen_gdn_chunked_avelang_v6
...
def test_qwen_gdn_chunked_v6_standalone_has_no_v3_v4_v5_imports():
    # standalone 模块只能依赖 torch、avelang 和 al，不能重新引入旧版本运行时依赖。
    source = Path(__file__).with_name("qwen_gdn_chunked_avelang_v6_standalone.py").read_text()
    assert "from qwen_gdn_chunked_avelang_v4" not in source
    assert "from qwen_gdn_chunked_avelang_v5" not in source
    assert "from qwen_gdn_chunk_cumsum_avelang_v3" not in source
    assert "import qwen_gdn_chunked_avelang_v4" not in source
    assert "import qwen_gdn_chunked_avelang_v5" not in source
    assert "import qwen_gdn_chunk_cumsum_avelang_v3" not in source
...
def test_qwen_gdn_chunked_v6_standalone_fp32_matches_ref_and_current_v6(
    case_name: str,
    batch_size: int,
    num_tokens: int,
    num_k_heads: int,
    ...
    # FP32 路径同时对齐 reference 和现有 v6，且比较完整五元组。
    ...
    expected_v6 = qwen_gdn_chunked_avelang_v6(
        q,
        k,
        v,
        g,
    ...
    actual = qwen_gdn_chunked_avelang_v6_standalone(
        q,
        k,
        v,
        g,
    ...
    assert_forward_close(
        actual,
        expected_v6,
        case_name=f"{case_name}:fp32_v6:prefer={prefer_optimized}",
        atol=FP32_ATOL,
        rtol=FP32_RTOL,
    )
...
@pytest.mark.parametrize("prefer_optimized", [True, False])
def test_qwen_gdn_chunked_v6_standalone_bf16_matches_quantized_ref_and_current_v6(
    case_name: str,
    batch_size: int,
    num_tokens: int,
    num_k_heads: int,
    ...
    assert_forward_close(
        actual,
        expected_v6,
        case_name=f"{case_name}:bf16_v6:prefer={prefer_optimized}",
        atol=BF16_ATOL,
        rtol=BF16_RTOL,
    )


def test_qwen_gdn_chunked_v6_standalone_rejects_mixed_qkv_dtype():
    ...
    with pytest.raises(ValueError):
        qwen_gdn_chunked_avelang_v6_standalone(q, k_fp32.contiguous(), v, g, beta, chunk_size=4)


@pytest.mark.parametrize("bad_name", ["q", "k", "v"])
def test_qwen_gdn_chunked_v6_standalone_rejects_non_contiguous_qkv(bad_name: str):
    # 非连续 q/k/v 会破坏当前 layout 假设，standalone wrapper 应直接拒绝。
    ...
    with pytest.raises(ValueError, match="contiguous"):
        qwen_gdn_chunked_avelang_v6_standalone(q, k, v, g, beta, chunk_size=4)
